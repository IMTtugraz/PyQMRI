...
__kernel void copy(__global float2 *out, __global float2 *in, const float scale, const int offset)
  {
    size_t x = get_global_id(0);
    size_t dimX = get_global_size(0);
    out[x] = in[dimX*offset+x]*scale;

  }
__kernel void copy_back(__global float2 *out, __global float2 *in, const float scale, const int offset)
  {
    size_t x = get_global_id(0);
    size_t dimX = get_global_size(0);
    out[dimX*offset+x] = in[x]*scale;
