...
import bSSFPVFA_model as bSSFP_model
...
def main(args):
    ...
    if args.streamed or args.slices > 5:
...
#### Close File after everything was read
    file.close()
#    data[10:] /= (np.linalg.norm(data[10:])/np.linalg.norm(data[:10]))
...
################################################################################
    ...
    ################################################################################
        model = bSSFP_model.bSSFP_Model(par.fa[:10],par.fa[10:],par.fa_corr,par.TR,images,NSlice,Nproj)
        opt.model = model
        opt.dz = 1
    ...
    ##IRGN Params
        ...
        opt.execute_3D()
        opt.result[:,1,...] = -par.TR[0]/np.log(opt.result[:,1,...])
...
## New .hdf5 save files #######################################################
###############################################################################
    outdir = time.strftime("%Y-%m-%d  %H-%M-%S_MRI_joint_"+args.type+"_"+name[:-3])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trafo', default=0, dest='trafo',  help='Choos between radial (1, default) and Cartesian (0) sampling. ')
