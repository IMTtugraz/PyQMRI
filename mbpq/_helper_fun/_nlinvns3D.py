...
def nlinvns(Y, n, *arg):  # *returnProfiles,**realConstr):
    ...
    if returnProfiles:
        R = np.zeros([c + 2, n, z, y, x], complex)

    else:
        R = np.zeros([2, n, z, y, x], complex)

    # initialization x-vector
    X0 = np.array(np.zeros([c + 1, z, y, x]), np.float64)
    X0[0, ...] = 1  # object part
    ...
    XT = np.zeros([c + 1, z, y, x], dtype=np.complex64)
    XN = np.copy(X0)
...
#    a = np.sqrt(si[M.ndim - 1]) * np.sqrt(si[M.ndim - 2])
    ...
    K = np.fft.ifft2(M, norm='ortho')
    return K  # .T


def weights(z, y, x):
    W = np.zeros([z, y, x],dtype=np.float32)
    for i in range(x):
        for j in range(y):
            for k in range(z):
                d = ((i) / x - 0.5)**2 + \
                    ((j) / y - 0.5)**2
                W[k, j, i] = 1 / (1 + 220 * d)**16  # 16
    return W
