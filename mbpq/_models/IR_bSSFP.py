...
class bSSFP_Model:
    def __init__(self, fa, fa_corr, TR, images, NSlice, Nproj):
        ...
        test_T1 = 1 / self.T1_sc * test_T1 * \
            np.ones((NSlice, dimY, dimX), dtype=DTYPE)
        test_T2 = 1 / self.T2_sc * test_T2 * \
            np.ones((NSlice, dimY, dimX), dtype=DTYPE)
        ...
        self.T1_sc = self.T1_sc * \
            np.linalg.norm(np.abs(DG_x[0, ...])) / np.linalg.norm(np.abs(DG_x[1, ...]))
        self.T2_sc = self.T2_sc * \
            np.linalg.norm(np.abs(DG_x[0, ...])) / np.linalg.norm(np.abs(DG_x[2, ...]))
        ...
        DG_x = self.execute_gradient_3D(
            np.array(
                [
                    ...
                    self.T2_sc],
                dtype=DTYPE))
        print('Grad Scaling init M0/T1: %f,  M0/T2: %f' %
              (np.linalg.norm(np.abs(DG_x[0, ...])) /
               np.linalg.norm(np.abs(DG_x[1, ...])), np.linalg.norm(np.abs(DG_x[0, ...])) /
               np.linalg.norm(np.abs(DG_x[2, ...]))))
        print('T1 scale: ', self.T1_sc, '/ T2_scale: ', self.T2_sc,
              '/ M0_scale: ', self.M0_sc)

        result = np.array([1 /
                           self.M0_sc *
                           np.ones((NSlice, dimY, dimX), dtype=DTYPE), 1 /
                           self.T1_sc *
                           800 *
                           np.ones((NSlice, dimY, dimX), dtype=DTYPE), 1 /
                           self.T2_sc *
                           80 *
                           np.ones((NSlice, dimY, dimX), dtype=DTYPE)], dtype=DTYPE)
        self.guess = result
        self.constraints.append(
            constraint(-10 / self.M0_sc, 10 / self.M0_sc, False))
        self.constraints.append(
            constraint(
                200 / self.T1_sc,
                5500 / self.T1_sc,
    ...
    def execute_forward_3D(self, x):
        ...
        tmp1 = M0 * M0_sc * (1 - E1)
        tmp2 = ((T1 * T1_sc / (T2 * T2_sc) - (T1 * T1_sc / (T2 * T2_sc) - 1)
                 * self.cos_phi + 1) * self.sin_phi_2 / self.sin_phi + 1)
        tmp3 = (self.sin_phi_2**2 / (T2 * T2_sc) +
                self.cos_phi_2**2 / (T1 * T1_sc))
        tmp4 = self.sin_phi / (-(-E2 + E1) * self.cos_phi + 1 - E1 * E2)
        for k in range(self.NScan):
            for l in range(self.Nproj):
                n = k * self.Nproj + l + 1
                S[k, l, ...] = tmp1 * \
                    (-tmp2 * np.exp(-TR * n * tmp3) + 1) * tmp4
    ...
    def execute_gradient_3D(self, x):
        ...
        tmp3 = (self.sin_phi_2**2 / (T2 * T2_sc) +
                self.cos_phi_2**2 / (T1 * T1_sc))
        tmp4 = self.sin_phi / (-(-E2 + E1) * self.cos_phi + 1 - E1 * E2)
        tmp5 = (TR * E1 * self.cos_phi / (T1**2 * T1_sc) +
                TR * E1 * E2 / (T1**2 * T1_sc)) * tmp4**2
        tmp6 = (-T1_sc * self.cos_phi / (T2 * T2_sc) + T1_sc / (T2 * T2_sc))
        tmp7 = E1 * self.sin_phi / \
            (T1**2 * T1_sc * (-(-E2 + E1) * self.cos_phi + 1 - E1 * E2))
        tmp8 = (-TR * E2 * self.cos_phi / (T2**2 * T2_sc) +
                TR * E1 * E2 / (T2**2 * T2_sc)) * tmp4**2
        tmp9 = (T1 * T1_sc * self.cos_phi / (T2**2 * T2_sc) -
                T1 * T1_sc / (T2**2 * T2_sc))
        ...
        for k in range(self.NScan):
            for l in range(self.Nproj):
                ...
                grad[1, k, l, ...] = M0 * tmp1 * tmp * tmp5 + M0 * tmp1 * \
                    (-tmp6 * Etmp * self.sin_phi_2 / self.sin_phi - TR * n * tmp2 * Etmp * self.cos_phi_2**2 / (T1**2 * T1_sc)) * tmp4 - M0 * M0_sc * TR * tmp * tmp7

                grad[2, k, l, ...] = M0 * tmp1 * tmp * tmp8 + M0 * tmp1 * \
                    (-tmp9 * Etmp * self.sin_phi_2 / self.sin_phi - TR * n * tmp2 * Etmp * self.sin_phi_2**2 / (T2**2 * T2_sc)) * tmp4
