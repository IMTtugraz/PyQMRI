...
class Model(BaseModel):
    ...
    def _execute_forward_3D(self, x):
        ...
        for j in range((self.t).size):
            ...
            ind_high = self.t[j] < (del_t+self.tau)
            ...
            if np.any(ind):
                S[j, ind] = 2*self.alpha[ind]*self.M0[ind]/self.lambd[ind]*f[ind]/(1/self.T1[ind]+f[ind]/self.lambd[ind]) * \
                           ...
                           (1-np.exp(-(self.t[j]-del_t[ind])*(1/self.T1[ind]+f[ind]/self.lambd[ind])))
            ind = self.t[j] >= del_t + self.tau
            if np.any(ind):
                S[j, ind] = 2*self.alpha[ind]*self.M0[ind]/self.lambd[ind]*f[ind]/(1/self.T1[ind]+f[ind]/self.lambd[ind]) * \
                            np.exp(-(del_t[ind])/self.T1b[ind]) * \
                            np.exp(-(self.t[j]-del_t[ind]-self.tau[ind]) *
                                (1/self.T1[ind]+f[ind]/self.lambd[ind])) * \
                            (1-np.exp(-self.tau[ind]*(1/self.T1[ind]+f[ind]/self.lambd[ind])))
    ...
    def _execute_gradient_3D(self, x):
        ...
        for j in range((self.t).size):
            ...
            ind_high = self.t[j] < (x[1, ...]*self.uk_scale[1]+self.tau)
            ind = ind_low & ind_high
            M0 = self.M0[ind]
            T1 = self.T1[ind]
            T1b = self.T1b[ind]
            ...
            if np.any(ind):
                grad[0, j, ind] = (-2*M0*f*f_sc**2*(del_t*del_t_sc - t[j]) *
                                  ...
                                  2*M0*f_sc*(-np.exp((del_t*del_t_sc - t[j]) *
                                                  (f*f_sc/lambd + 1/T1)) + 1) *
                                  np.exp(-del_t*del_t_sc/T1b) /
            ...
            ind = self.t[j] >= x[1, ...]*self.uk_scale[1] + self.tau
            M0 = self.M0[ind]
            T1 = self.T1[ind]
            T1b = self.T1b[ind]
            lambd = self.lambd[ind]
    ...
    def plot_unknowns(self, x, dim_2D=False):
        ...
        if dim_2D:
            ...
            if not self.figure:
                ...
                self.ax[1].set_title('f', color='white')
                ...
                self.ax[3].set_title('del_t', color='white')
                ...
                self.del_t_plot_cor.set_data((del_t[:, int(del_t.shape[1] / 2), ...]))
    ...
    def _set_init_scales(self, images):
        test_f = 100 * np.ones((self.NSlice, self.dimY, self.dimX), dtype=DTYPE)
        test_del_t = 1 * np.ones(
            (self.NSlice, self.dimY, self.dimX), dtype=DTYPE)
        x = np.array([test_f / self.uk_scale[0],
                      test_del_t / self.uk_scale[1]], dtype=DTYPE)
        return x
