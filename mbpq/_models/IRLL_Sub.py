...
class Model(BaseModel):
    def __init__(self, par, images):
        ...
        self.constraints.append(constraints(
            np.exp(-self.scale / 10) / self.uk_scale[1], np.exp(-self.scale / 5500) / self.uk_scale[1], True))
    ...
    def _execute_forward_2D(self, x, islice):
        ...
        Q = (-Etr * Etd * F * (-(Etau * cos_phi)**(N - 1) + 1) * cos_phi + Etr *
             Etd - 2 * Etd + 1) / (Etr * Etd * (Etau * cos_phi)**(N - 1) * cos_phi + 1)
        Q_F = Q - F
    ...
    def _execute_gradient_2D(self, x, islice):
        ...
        F = (1 - Etau) / (1 - Etau * cos_phi)
        Q = (-Etr * Etd * (-Etau + 1) * (-(Etau * cos_phi)**(N - 1) + 1) * cos_phi / (-Etau *
                                                                                      cos_phi + 1) + Etr * Etd - 2 * Etd + 1) / (Etr * Etd * (Etau * cos_phi)**(N - 1) * cos_phi + 1)
        Q_F = Q - F
        ...
        tmp2 = tau * Etau * (-Etau + 1) * cos_phi / (x[1, ...] * scale * (
            -Etau * cos_phi + 1)**2) - tau * Etau / (x[1, ...] * scale * (-Etau * cos_phi + 1))

        tmp3 = (-(-Etau + 1) / (-Etau * cos_phi + 1) + (-Etr * Etd * (-Etau + 1) * (-(Etau * cos_phi)**(N - 1) + 1) * cos_phi / \
                (-Etau * cos_phi + 1) + Etr * Etd - 2 * Etd + 1) / (Etr * Etd * (Etau * cos_phi)**(N - 1) * cos_phi + 1)) / (x[1, ...] * scale)
        for i in range(self.NLL):
            for j in range(self.Nproj):
                n = i * self.Nproj + j + 1
                ...
                grad[1, i, j, ...] = M0 * M0_sc * ((Etau * cos_phi)**(
                    n - 1) * tmp1 + tmp2 + tau * (Etau * cos_phi)**(n - 1) * (n - 1) * tmp3) * sin_phi
    ...
    def _execute_gradient_3D(self, x):
        ...
        def numexpeval_T1(
                ...
                tau):
            return ne.evaluate(
                "M0*M0_sc*((Etau*cos_phi)**(n - 1)*tmp1 + tmp2 + tau*(Etau*cos_phi)**(n - 1)*(n - 1)*tmp3)*sin_phi")
        ...
        for i in range(self.NLL):
            for j in range(self.Nproj):
                ...
                grad[1, i, j, ...] = numexpeval_T1(
                    M0, M0_sc, Etau, cos_phi, sin_phi, n, tmp1, tmp2, tmp3, tau)
...
#        print('Grad Scaling', np.linalg.norm(np.abs(np.mean(grad,axis=2)[0,...]))/np.linalg.norm(np.abs(np.mean(grad,axis=2)[1,...])))
    ...
    def _set_init_scales(self):
        ...
        test_T1 = np.exp(-self.scale / np.reshape(np.linspace(10,
                                                              5500,
                                                              self.dimX * self.dimY * self.NSlice),
                                                  (self.NSlice,
                                                   self.dimY,
                                                   self.dimX)))
        ...
        G_x = self._execute_forward_3D(
            np.array([test_M0, test_T1], dtype=DTYPE))

        self.uk_scale[0] *= 1 / np.median(np.abs(G_x))

        DG_x = self._execute_gradient_3D(
            np.array([test_M0 / self.uk_scale[0], test_T1], dtype=DTYPE))
...
#    DG_x =  self._execute_gradient_3D(np.array([test_M0/self.uk_scale[0],test_T1],dtype=DTYPE))
        ...
        return np.array([1 / self.uk_scale[0] * np.ones((self.NSlice,
                                                         self.dimY,
                                                         self.dimX),
                                                        dtype=DTYPE),
                         np.exp(-self.scale / 800) / self.uk_scale[1] * np.ones((self.NSlice,
                                                                                 self.dimY,
                                                                                 self.dimX),
                                                                                dtype=DTYPE)])
