    ...
    def __init__(self, par, trafo=1, imagespace=False, SMS=0):
        ...
        if imagespace:
            ...
            if SMS:
                self.tmp_sino = clarray.empty(
                    self.queue,
                    (self.NScan, self.NC, par["packs"], self.Nproj, self.N),
                    DTYPE, "C")
            else:
                self.tmp_sino = clarray.empty(
                    self.queue,
...
###############################################################################
    ...
    def tgv_solve_3D_explicit(self, x, res, iters):
        ...
        AT.add_event(self.operator_forward(x, AT))
        ATd.add_event(self.operator_adjoint(res, ATd))
        ...
        for i in range(iters):
            ...
            AT.add_event(self.operator_forward(x_new, AT))
