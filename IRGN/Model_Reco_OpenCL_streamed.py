...
class Model_Reco:
  def __init__(self,par,trafo=1,imagespace=False, SMS=False):
    ...
    for j in range(self.num_dev):
      ...
      self.tmp_img.append(clarray.empty(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),DTYPE,"C"))
...
#      grad.add_event(self.f_grad(grad,x,idx=i,wait_for=grad.events+x.events))
#      grad = grad.get()
#      scale = np.reshape(np.transpose(grad,(1,0,2,3,4)),(self.unknowns,self.NSlice*self.dimY*self.dimX*4))
...
#### Allocate temporary Arrays
    (Axold_part,Kyk1_part,Kyk2_part,xk_part,v_part,res_part,z1_new_part,z2_new_part,r_new_part,Kyk1_new_part,Kyk2_new_part,
            x_new_part,Ax_part,v_new_part,gradx_part,gradx_xold_part,symgrad_v_part,symgrad_v_vold_part,
            x_part,r_part,z1_part,z2_part) = \
        self.preallocate_space(TGV=True)


##### Warmup
    self.stream_initial((x,r,z1,z2,Axold,Kyk1,Kyk2,gradx_xold, symgrad_v_vold,v),
                        (x_part,r_part,z1_part,z2_part,Axold_part,Kyk1_part,Kyk2_part,gradx_xold_part,symgrad_v_vold_part, v_part),
                        TGV=True)
##### Start Iterations
    for myit in range(iters):
      self.stream_primal_update((x_new,xk,x,v_new,v,Kyk1,gradx,Ax,Kyk2,symgrad_v),
                                (x_new_part,xk_part,x_part,v_new_part,v_part,Kyk1_part,gradx_part,Ax_part,Kyk2_part,symgrad_v_part),tau,delta,TGV=True)
      ...
      while True:
        ...
        (ynorm,lhs) = self.stream_dual_update((z1,gradx,gradx_xold,v_new,v,r,Ax,Axold,res,Kyk1,
         z1_new,r_new,Kyk1_new,z2,symgrad_v,symgrad_v_vold,Kyk2,z2_new,Kyk2_new),
                                              (z1_part,z1_new_part,z2_part,z2_new_part,gradx_part,
                                               ...
                                               Kyk1_new_part,Kyk2_part,Kyk2_new_part,symgrad_v_part,symgrad_v_vold_part),
                                               (ynorm,lhs,beta_line,tau_new,alpha,beta,theta_line),TGV=True)
      ...
      (Kyk1, Kyk1_new, Kyk2, Kyk2_new, Axold, Ax, z1, z1_new, z2, z2_new, r, r_new, gradx_xold, gradx, symgrad_v_vold, symgrad_v) =\
      (Kyk1_new, Kyk1, Kyk2_new, Kyk2, Ax, Axold, z1_new, z1, z2_new, z2, r_new, r, gradx, gradx_xold, symgrad_v,symgrad_v_vold)
      tau =  (tau_new)
      ...
      if not np.mod(myit,10):
        ...
        if np.abs(primal-primal_new)<(self.irgn_par["lambd"]*self.NSlice)*self.irgn_par["tol"]:
          print("Terminated at iteration %d because the energy decrease in the primal problem was less than %.3e"%(myit,abs(primal-primal_new).get()/(self.irgn_par["lambd"]*self.NSlice)))
          self.v = v_new.get()
          self.r = r.get()
          self.z1 = z1.get()
          self.z2 = z2.get()
        ...
        if (gap > gap_min*self.irgn_par["stag"]) and myit>1:
          ...
          return x_new.get()
        if np.abs((gap-gap_old)/gap_min)<self.irgn_par["tol"] and myit>1:
          self.v = v_new.get()
          self.r = r.get()
          self.z1 = z1.get()
          self.z2 = z2.get()
          print("Terminated at iteration %d because the relative energy decrease of the PD gap was less than %.3e"%(myit,abs((gap-gap_old)/gap_min).get()))
          return x_new.get()
        primal = primal_new
        gap_old = gap
        sys.stdout.write("Iteration: %04d ---- Primal: %2.2e, Dual: %2.2e, Gap: %2.2e \r"%(myit,primal.get()/(self.irgn_par["lambd"]*self.NSlice),dual.get()/(self.irgn_par["lambd"]*self.NSlice),gap.get()/(self.irgn_par["lambd"]*self.NSlice)))
        sys.stdout.flush()
...
#### Allocate temporary Arrays
    (Axold_part,Kyk1_part,xk_part,res_part,z1_new_part,r_new_part,Kyk1_new_part,
              x_new_part,Ax_part,gradx_part,gradx_xold_part,
              x_part,r_part,z1_part) = self.preallocate_space(TGV=False)

##### Warmup
    self.stream_initial((x,r,z1,Axold,Kyk1,gradx_xold),
                        (x_part,r_part,z1_part,Axold_part,Kyk1_part,gradx_xold_part),
                        TGV=False)

    for myit in range(iters):
  #### Allocate temporary Arrays
      self.stream_primal_update((x_new,xk,x,Kyk1,gradx,Ax),
                                (x_new_part,xk_part,x_part,Kyk1_part,gradx_part,Ax_part),tau,delta,TGV=False)
      ...
      while True:
        ...
        (ynorm,lhs) = self.stream_dual_update((z1,gradx,gradx_xold,r,Ax,Axold,res,Kyk1,
         z1_new,r_new,Kyk1_new),
                                              (z1_part,z1_new_part,gradx_part,
                                               gradx_xold_part,
                                               r_part,r_new_part,Ax_part,Axold_part,res_part,Kyk1_part,
                                               Kyk1_new_part),
                                               (ynorm,lhs,beta_line,tau_new,alpha,theta_line),TGV=False)
  ...
  def FT_streamed(self,outp,inp):
      ...
      j=0
      for i in range(self.num_dev):
        cl_out.append(clarray.empty(self.queue[3*i],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_out.append(clarray.empty(self.queue[3*i+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
      cl_data = []
      for i in range(self.num_dev):
  ...
  def operator_forward_streamed(self, outp, inp):
      ...
      for i in range(self.num_dev):
        cl_out.append(clarray.empty(self.queue[3*i],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_out.append(clarray.empty(self.queue[3*i+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_tmp.append(clarray.empty(self.queue[3*i],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
        cl_tmp.append(clarray.empty(self.queue[3*i+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
  ...
  def preallocate_space(self,TGV=True):
    ...
    for i in range(2*self.num_dev):
      Axold_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      Kyk1_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      xk_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      res_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      z1_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      z1_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      r_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      r_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      Kyk1_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      x_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      x_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      Ax_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      gradx_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      gradx_xold_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
      if TGV:
        v_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        v_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        z2_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        z2_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        symgrad_v_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        symgrad_v_vold_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        Kyk2_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))
        Kyk2_new_part.append(clarray.empty(self.queue[3*int(np.mod(i,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(i,self.num_dev))]))


    if TGV:
      return (Axold_part,Kyk1_part,Kyk2_part,xk_part,v_part,res_part,z1_new_part,z2_new_part,r_new_part,Kyk1_new_part,Kyk2_new_part,
              x_new_part,Ax_part,v_new_part,gradx_part,gradx_xold_part,symgrad_v_part,symgrad_v_vold_part,
              x_part,r_part,z1_part,z2_part)
    else:
      return (Axold_part,Kyk1_part,xk_part,res_part,z1_new_part,r_new_part,Kyk1_new_part,
              x_new_part,Ax_part,gradx_part,gradx_xold_part,
              x_part,r_part,z1_part)


  def stream_initial(self,arrays,parts,TGV=True):
    if TGV:
      (x,r,z1,z2,Axold,Kyk1,Kyk2,gradx_xold, symgrad_v_vold,v) = arrays
      (x_part,r_part,z1_part,z2_part,Axold_part,Kyk1_part,Kyk2_part,gradx_xold_part,symgrad_v_vold_part, v_part) = parts
    else:
      (x,r,z1,Axold,Kyk1,gradx_xold) = arrays
      (x_part,r_part,z1_part,Axold_part,Kyk1_part,gradx_xold_part) = parts
    j=0
    last=0
    for i in range(self.num_dev):
    ...
    for i in range(self.num_dev):
      ...
      if idx_start==0:
        ...
        idx_start-=self.overlap
      x_part[self.num_dev+i].add_event(cl.enqueue_copy(self.queue[3*i+1],x_part[self.num_dev+i].data,x[idx_start:idx_stop,...],wait_for=x_part[self.num_dev+i].events,is_blocking=False))
      r_part[self.num_dev+i].add_event(cl.enqueue_copy(self.queue[3*i+1],r_part[self.num_dev+i].data,r[idx_start:idx_stop,...],wait_for=r_part[self.num_dev+i].events,is_blocking=False))
...
#### Stream
    for j in range(2*self.num_dev,int(self.NSlice/(2*self.par_slices*self.num_dev)+(2*self.num_dev-1))):
        ...
        for i in range(self.num_dev):
          ...
          idx_stop = (i+1)*self.par_slices+(2*self.num_dev*(j-2*self.num_dev)+self.num_dev)*self.par_slices+self.overlap
          if TGV:
            Kyk2_part[i+self.num_dev].add_event(cl.enqueue_copy(self.queue[3*i+2],Kyk2[idx_start:idx_stop,...],Kyk2_part[i+self.num_dev].data,wait_for=Kyk2_part[i+self.num_dev].events,is_blocking=False))
...
#### Collect last block
  ...
  def stream_primal_update(self,arrays,parts,tau,delta,TGV=True):
    ...
    if TGV:
      ...
      (x_new_part,xk_part,x_part,v_new_part,v_part,Kyk1_part,gradx_part,Ax_part,Kyk2_part,symgrad_v_part)=parts
    else:
      (x_new,xk,x,Kyk1,gradx,Ax) = arrays
      (x_new_part,xk_part,x_part,Kyk1_part,gradx_part,Ax_part)=parts
    for i in range(self.num_dev):
      idx_start = i*self.par_slices
      idx_stop = (i+1)*self.par_slices+self.overlap
      x_part[i].add_event(cl.enqueue_copy(self.queue[3*i],x_part[i].data,x[idx_start:idx_stop,...],wait_for=x_part[i].events,is_blocking=False))
...
#### Collect last block
  ...
  def stream_dual_update(self,arrays,parts,pars,TGV=True):
    if TGV:
      (ynorm,lhs,beta_line,tau_new,alpha,beta,theta_line) = pars
      (z1,gradx,gradx_xold,v_new,v,r,Ax,Axold,res,Kyk1,
       z1_new,r_new,Kyk1_new,z2,symgrad_v,symgrad_v_vold,Kyk2,z2_new,Kyk2_new) = arrays
      (z1_part,z1_new_part,z2_part,z2_new_part,gradx_part,gradx_xold_part,v_new_part,v_part,
       r_part,r_new_part,Ax_part,Axold_part,res_part,Kyk1_part,Kyk1_new_part,Kyk2_part,Kyk2_new_part,
      ...
      (z1_part,z1_new_part,gradx_part,gradx_xold_part,
       r_part,r_new_part,Ax_part,Axold_part,res_part,Kyk1_part,Kyk1_new_part) = parts
    ...
    for i in range(self.num_dev):
      ...
      Axold_part[i].add_event(cl.enqueue_copy(self.queue[3*i],Axold_part[i].data,Axold[idx_start:idx_stop,...],wait_for=Axold_part[i].events,is_blocking=False))
      ...
      r_new_part[i].add_event(self.update_r(r_new_part[i],r_part[i],Ax_part[i],Axold_part[i],res_part[i],beta_line*tau_new,theta_line,self.irgn_par["lambd"],i,0))
    ...
    for i in range(self.num_dev):
      ...
      Axold_part[i+self.num_dev].add_event(cl.enqueue_copy(self.queue[3*i+1],Axold_part[i+self.num_dev].data,Axold[idx_start:idx_stop,...],wait_for=Axold_part[i+self.num_dev].events,is_blocking=False))
      ...
      r_new_part[i+self.num_dev].add_event(self.update_r(r_new_part[i+self.num_dev],r_part[self.num_dev+i],Ax_part[self.num_dev+i],Axold_part[self.num_dev+i],res_part[self.num_dev+i],beta_line*tau_new,theta_line,self.irgn_par["lambd"],i,1))
...
##    for i in range(self.num_dev):
#      (self.bdiv(x_part[i+self.num_dev],gradx_part[i+self.num_dev],i,1,last=last)).wait()
#  ### Stream
#    for j in range(2*self.num_dev,int(self.NSlice/(2*self.par_slices*self.num_dev)+(2*self.num_dev-1))):
#        for i in range(self.num_dev):
#          ## Get Data
