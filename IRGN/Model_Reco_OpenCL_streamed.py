...
class Model_Reco:
        ...
        for j in range(self.num_dev):
            ...
            for i in range(2):
                ...
                self.ukscale.append(
                    clarray.to_device(
                        self.queue[3*j+i],
                        np.ones(self.unknowns, dtype=DTYPE_real)))
            self.prg.append(
                Program(
                    self.ctx[j],
    ...
    def operator_forward_full(self, out, x, idx=0, idxq=0, wait_for=[]):
        self.tmp_img[2*idx+idxq].add_event(
            self.eval_fwd_streamed(
                ...
                idx, idxq, wait_for=self.tmp_img[2*idx+idxq].events+x.events))
    ...
    def operator_adjoint_full(self, out, x, z,
        ...
        return self.prg[idx].update_Kyk1(
            ...
            np.int32(self.NScan), self.ukscale[2*idx+idxq].data,
            np.float32(np.amax(self.ukscale[2*idx+idxq].get())),
    ...
    def eval_fwd_streamed(self, y, x, idx=0, idxq=0, wait_for=[]):
        return self.prg[idx].operator_fwd(
            self.queue[3*idx+idxq],
            (self.par_slices+self.overlap, self.dimY, self.dimX), None,
            y.data, x.data, self.coil_buf_part[idx+idxq*self.num_dev].data,
            self.grad_buf_part[idx+idxq*self.num_dev].data,
            np.int32(self.NC), np.int32(self.par_slices+self.overlap),
    ...
    def update_z1(self, z_new, z, gx, gx_, vx, vx_,
                  sigma, theta, alpha, idx=0, idxq=0, wait_for=[]):
    ...
    def update_r(self, r_new, r, A, A_, res,
        ...
        return self.prg[idx].update_r(
            self.queue[3*idx+idxq], (r.size,), None, r_new.data, r.data,
    ...
    def update_v(self, v_new, v, Kyk2, tau,  idx=0, idxq=0, wait_for=[]):
        return self.prg[idx].update_v(
            self.queue[3*idx+idxq], (v[..., 0].size,), None,
    ...
    def eval_const(self):
        ...
        for j in range(self.num_dev):
            self.min_const.append(
                clarray.to_device(self.queue[3*j], min_const))
            self.max_const.append(
                clarray.to_device(self.queue[3*j], max_const))
            self.real_const.append(
                clarray.to_device(self.queue[3*j], real_const))
    ...
    def execute_3D(self, TV=0):
        ...
        for i in range(self.irgn_par["max_gn_it"]):
            ...
            if not np.mod(i, 1):
                ...
                scale = 1/scale
                print("Inverse Scale of the model "
                      "Gradients w.r.t the maximum: \n", scale)
                tmp_sum = 0
            ...
            if (np.abs(self.fval_min-self.fval) <
               self.irgn_par["lambd"]*self.irgn_par["tol"]) and i > 0:
                print("Terminated at GN-iteration %d because the energy "
                      "decrease was less than %.3e" %
...
###############################################################################
    ...
    # Start Iterations
        for myit in range(iters):
            ...
            while True:
                ...
                (ynorm, lhs) = self.stream_dual_update(
                    ...
                    (z1_part, z1_new_part, z2_part, z2_new_part, gradx_part,
                     ...
                     symgrad_v_vold_part),
                    (ynorm, lhs, beta_line, tau_new, alpha, beta, theta_line),
                    TGV=True)
            ...
            if not np.mod(myit, 10):
                ...
                primal_new = (
                    self.irgn_par["lambd"]/2*np.vdot(Axold-res, Axold-res) +
                ...
                dual = (
                    ...
                    - np.vdot(xk.flatten(), (-Kyk1).flatten()) + np.sum(Kyk2)
                ...
                if np.abs(primal-primal_new) <\
                   (self.irgn_par["lambd"]*self.NSlice)*self.irgn_par["tol"]:
                    print("Terminated at iteration %d because the energy "
                          ...
                          (myit, abs(primal-primal_new).get() /
                           (self.irgn_par["lambd"]*self.NSlice)))
                    self.v = v_new
                    self.r = r
                    self.z1 = z1
                ...
                sys.stdout.write(
                    ...
                    % (myit, primal/(self.irgn_par["lambd"]*self.NSlice),
                       dual/(self.irgn_par["lambd"]*self.NSlice),
                       gap/(self.irgn_par["lambd"]*self.NSlice)))
                sys.stdout.flush()
            (x, x_new) = (x_new, x)
            (v, v_new) = (v_new, v)
        ...
        for myit in range(iters):
            ...
            while True:
                ...
                (ynorm, lhs) = self.stream_dual_update(
                    ...
                    (ynorm, lhs, beta_line, tau_new, alpha, theta_line),
                    TGV=False)
                ...
                if np.sqrt(beta_line)*tau_new*(abs(lhs)**(1/2)) <= \
                   (abs(ynorm)**(1/2))*delta_line:
                    ...
                    tau_new = tau_new*mu_line
            (Kyk1, Kyk1_new,  Axold, Ax, z1, z1_new, r, r_new, tau) = (
             Kyk1_new, Kyk1,  Ax, Axold, z1_new, z1, r_new, r, tau_new)
    ...
    def FT_streamed(self, outp, inp):
        ...
        for i in range(self.num_dev):
            ...
            cl_data.append(clarray.to_device(
                self.queue[3*i], inp[idx_start:idx_stop, ...]))
        ...
        for i in range(self.num_dev):
            ...
            cl_data.append(clarray.to_device(
                self.queue[3*i+1], inp[idx_start:idx_stop, ...]))
        ...
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                cl_out[2*i+1].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], outp[idx_start:idx_stop, ...],
                ...
                cl_data[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], cl_data[i+self.num_dev].data,
                        inp[idx_start:idx_stop, ...],
    ...
    def operator_forward_streamed(self, outp, inp):
        ...
        for i in range(self.num_dev):
            ...
            self.coil_buf_part.append(
                clarray.to_device(
                    self.queue[3*i], self.C[idx_start:idx_stop, ...],
            ...
            cl_tmp[2*i].add_event(
                self.eval_fwd_streamed(
                    ...
                    wait_for=(
                        cl_tmp[2*i].events+cl_data[i].events +
                        self.coil_buf_part[i].events +
                        self.grad_buf_part[i].events)))
            cl_out[2*i].add_event(
                self.NUFFT[2*i].fwd_NUFFT(
        ...
        for i in range(self.num_dev):
            ...
            self.coil_buf_part.append(
                clarray.to_device(
                    self.queue[3*i+1], self.C[idx_start:idx_stop, ...],
                    allocator=self.alloc[i]))
            self.grad_buf_part.append(
                clarray.to_device(
                    self.queue[3*i+1],
                    self.grad_x[idx_start:idx_stop, ...],
                    allocator=self.alloc[i]))
            cl_tmp[2*i+1].add_event(
                self.eval_fwd_streamed(
                    cl_tmp[2*i+1], cl_data[self.num_dev+i], idx=i, idxq=1,
                    wait_for=(cl_tmp[2*i+1].events +
                              cl_data[self.num_dev+i].events +
                              self.coil_buf_part[self.num_dev+i].events +
                              self.grad_buf_part[self.num_dev+i].events)))
            cl_out[2*i+1].add_event(
                self.NUFFT[2*i+1].fwd_NUFFT(
                    cl_out[2*i+1], cl_tmp[2*i+1],
                    wait_for=cl_tmp[2*i+1].events+cl_out[2*i+1].events))
        ...
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                self.grad_buf_part[i] = (clarray.to_device(
                    self.queue[3*i], self.grad_x[idx_start:idx_stop, ...]))
                cl_tmp[2*i].add_event(
                    self.eval_fwd_streamed(
                        cl_tmp[2*i], cl_data[i], idx=i, idxq=0,
                        wait_for=(cl_tmp[2*i].events+cl_data[i].events +
                                  self.coil_buf_part[i].events +
                                  self.grad_buf_part[i].events)))
                cl_out[2*i].add_event(
                    self.NUFFT[2*i].fwd_NUFFT(
            ...
            for i in range(self.num_dev):
                ...
                self.coil_buf_part[self.num_dev+i] = (
                    clarray.to_device(self.queue[3*i+1],
    ...
    def preallocate_space(self, TGV=True):
        ...
        for i in range(2*self.num_dev):
            Axold_part.append(
                clarray.empty(
                    self.queue[3*int(np.mod(i, self.num_dev))],
                    (self.par_slices+self.overlap, self.NScan,
                     self.NC, self.Nproj, self.N),
                    dtype=DTYPE,
        ...
        for i in range(self.num_dev):
            ...
            if TGV:
                v_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], v_part[i+self.num_dev].data,
                        v[idx_start:idx_stop, ...],
    ...
    # Stream
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                Axold_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], Axold[idx_start:idx_stop, ...],
                        Axold_part[i].data,
                        wait_for=Axold_part[i].events, is_blocking=False))
                Kyk1_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], Kyk1[idx_start:idx_stop, ...],
                        Kyk1_part[i].data,
            ...
            for i in range(self.num_dev):
                ...
                self.coil_buf_part[self.num_dev+i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], self.coil_buf_part[
                            self.num_dev+i].data,
                        self.C[idx_start:idx_stop, ...],
                        wait_for=self.coil_buf_part[self.num_dev+i].events,
                        is_blocking=False))
    ...
    # Warmup
        ...
        for i in range(self.num_dev):
            ...
            x_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], x_part[i+self.num_dev].data,
                    x[idx_start:idx_stop, ...],
            ...
            if TGV:
                ...
                z2_part[self.num_dev+i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], z2_part[self.num_dev+i].data,
                        z2[idx_start:idx_stop, ...],
        ...
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                if TGV:
                    Kyk2_part[i].add_event(
                        cl.enqueue_copy(
                            self.queue[3*i+2], Kyk2[idx_start:idx_stop, ...],
                            Kyk2_part[i].data,
                            wait_for=Kyk2_part[i].events,
                            is_blocking=False))
                gradx_xold_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], gradx_xold[idx_start:idx_stop, ...],
    ...
    def stream_primal_update(self, arrays, parts, tau, delta, TGV=True):
        ...
        for i in range(self.num_dev):
            ...
            Kyk1_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], Kyk1_part[i].data,
                    Kyk1[idx_start:idx_stop, ...],
                    wait_for=Kyk1_part[i].events, is_blocking=False))
            xk_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], xk_part[i].data,
    ...
    # Stream
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                x_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], x_new[idx_start:idx_stop, ...],
                        x_new_part[i].data,
                        wait_for=x_new_part[i].events, is_blocking=False))
                gradx_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], gradx[idx_start:idx_stop, ...],
                        gradx_part[i].data,
                        wait_for=gradx_part[i].events, is_blocking=False))
                Ax_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], Ax[idx_start:idx_stop, ...],
                        Ax_part[i].data,
    ...
    # Collect last block
        ...
        if TGV:
            ...
            for i in range(self.num_dev):
                idx_start = (self.NSlice -
                             ((i+1)*self.par_slices)-self.overlap)
                idx_stop = (self.NSlice-(i*self.par_slices))
                v_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], v_part[i].data,
                ...
                Kyk2_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], Kyk2_part[i].data,
            ...
            for i in range(self.num_dev):
                ...
                idx_stop = (self.NSlice -
                            ((i+1+self.num_dev-1)*self.par_slices))
                if idx_start == 0:
                    idx_stop += self.overlap
                else:
                    idx_start -= self.overlap
                ...
                Kyk2_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], Kyk2_part[i+self.num_dev].data,
                        Kyk2[idx_start:idx_stop, ...],
                        wait_for=Kyk2_part[i+self.num_dev].events,
                        is_blocking=False))
            for i in range(self.num_dev):
        ...
        # Stream
            for j in range(2*self.num_dev,
                ...
                for i in range(self.num_dev):
                    ...
                    v_new_part[i].add_event(
                        cl.enqueue_copy(
                            self.queue[3*i+2], v_new[idx_start:idx_stop, ...],
                            v_new_part[i].data,
        ...
        # Collect last block
            ...
            for i in range(self.num_dev):
                ...
                symgrad_v_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], symgrad_v[idx_start:idx_stop, ...],
                        symgrad_v_part[i].data,
                        wait_for=symgrad_v_part[i].events,
                        is_blocking=False))
            for i in range(self.num_dev):
    ...
    def stream_dual_update(self, arrays, parts, pars, TGV=True):
        ...
        for i in range(self.num_dev):
            ...
            gradx_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], gradx_part[i].data,
                    gradx[idx_start:idx_stop, ...],
                    wait_for=gradx_part[i].events, is_blocking=False))
            gradx_xold_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], gradx_xold_part[i].data,
            ...
            Ax_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], Ax_part[i].data,
                    Ax[idx_start:idx_stop, ...],
                    wait_for=Ax_part[i].events, is_blocking=False))
            Axold_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], Axold_part[i].data,
                    Axold[idx_start:idx_stop, ...],
                    wait_for=Axold_part[i].events, is_blocking=False))
            res_part[i].add_event(
                cl.enqueue_copy(
                    self.queue[3*i], res_part[i].data,
            ...
            if TGV:
                v_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], v_new_part[i].data,
            ...
            if TGV:
                z1_new_part[i].add_event(
                    self.update_z1(
                        ...
                        beta_line*tau_new, theta_line, alpha, i, 0))
            else:
                z1_new_part[i].add_event(
                    self.update_z1_tv(
                        z1_new_part[i], z1_part[i], gradx_part[i],
        ...
        for i in range(self.num_dev):
            ...
            z1_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], z1_part[i+self.num_dev].data,
                    z1[idx_start:idx_stop, ...],
                    wait_for=z1_part[i+self.num_dev].events,
                    is_blocking=False))
            gradx_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], gradx_part[i+self.num_dev].data,
                    gradx[idx_start:idx_stop, ...],
                    wait_for=gradx_part[i+self.num_dev].events,
                    is_blocking=False))
            gradx_xold_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], gradx_xold_part[i+self.num_dev].data,
                    gradx_xold[idx_start:idx_stop, ...],
                    wait_for=gradx_xold_part[i+self.num_dev].events,
                    is_blocking=False))
            r_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], r_part[i+self.num_dev].data,
                    r[idx_start:idx_stop, ...],
                    wait_for=r_part[i+self.num_dev].events, is_blocking=False))
            Ax_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], Ax_part[i+self.num_dev].data,
                    Ax[idx_start:idx_stop, ...],
                    wait_for=Ax_part[i+self.num_dev].events,
                    is_blocking=False))
            Axold_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], Axold_part[i+self.num_dev].data,
                    Axold[idx_start:idx_stop, ...],
                    wait_for=Axold_part[i+self.num_dev].events,
                    is_blocking=False))
            res_part[i+self.num_dev].add_event(
                cl.enqueue_copy(
                    self.queue[3*i+1], res_part[i+self.num_dev].data,
                    res[idx_start:idx_stop, ...],
            ...
            if TGV:
                v_new_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], v_new_part[i+self.num_dev].data,
                        v_new[idx_start:idx_stop, ...],
                        wait_for=v_new_part[i+self.num_dev].events,
                        is_blocking=False))
                v_part[i+self.num_dev].add_event(
            ...
            if TGV:
                z1_new_part[i+self.num_dev].add_event(
                    self.update_z1(
                        ...
                        beta_line*tau_new, theta_line, alpha, i, 1))
            else:
                z1_new_part[i+self.num_dev].add_event(
                    self.update_z1_tv(
                        z1_new_part[i+self.num_dev], z1_part[self.num_dev+i],
        ...
        for j in range(2*self.num_dev,
            ...
            for i in range(self.num_dev):
                ...
                z1_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], z1_new[idx_start:idx_stop, ...],
                        z1_new_part[i].data,
                        wait_for=z1_new_part[i].events, is_blocking=False))
                r_new_part[i].add_event(
                    cl.enqueue_copy
                    (self.queue[3*i+2], r_new[idx_start:idx_stop, ...],
                     r_new_part[i].data,
                     wait_for=r_new_part[i].events, is_blocking=False))
                Kyk1_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2], Kyk1_new[idx_start:idx_stop, ...],
        ...
        if TGV:
            ...
            for i in range(self.num_dev):
                ...
                symgrad_v_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], symgrad_v_part[i].data,
                        symgrad_v[idx_start:idx_stop, ...],
                        wait_for=symgrad_v_part[i].events, is_blocking=False))
                symgrad_v_vold_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], symgrad_v_vold_part[i].data,
                        symgrad_v_vold[idx_start:idx_stop, ...],
                        wait_for=symgrad_v_vold_part[i].events,
                        is_blocking=False))
                Kyk2_part[i].add_event(
                ...
                z1_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i], z1_new_part[i].data,
            ...
            for i in range(self.num_dev):
                ...
                z2_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], z2_part[i+self.num_dev].data,
                        z2[idx_start:idx_stop, ...],
                        wait_for=z2_part[i+self.num_dev].events,
                        is_blocking=False))
                symgrad_v_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], symgrad_v_part[i+self.num_dev].data,
                        symgrad_v[idx_start:idx_stop, ...],
                        wait_for=symgrad_v_part[i+self.num_dev].events,
                        is_blocking=False))
                symgrad_v_vold_part[i+self.num_dev].add_event(
                ...
                z1_new_part[i+self.num_dev].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+1], z1_new_part[i+self.num_dev].data,
                        z1_new[idx_start:idx_stop, ...],
                        wait_for=z1_new_part[i+self.num_dev].events,
                        is_blocking=False))
            for i in range(self.num_dev):
            ...
            for j in range(2*self.num_dev,
                ...
                for i in range(self.num_dev):
                    ...
                    z2_new_part[i].add_event(
                        cl.enqueue_copy(
                            self.queue[3*i+2], z2_new[idx_start:idx_stop, ...],
                            z2_new_part[i].data,
            ...
            for i in range(self.num_dev):
                ...
                Kyk2_new_part[i].add_event(
                    cl.enqueue_copy(
                        self.queue[3*i+2],  Kyk2_new[idx_start:idx_stop, ...],
                        Kyk2_new_part[i].data,
                        wait_for=Kyk2_new_part[i].events,
                        is_blocking=False))
                ynorm += ((
