...
#    print(test_M0)
    ...
    ADC_xy = 1/self.uk_scale[1]*ADC*np.ones((Nislice,dimY,dimX),dtype=DTYPE)
...
#    kurt_z = 1/self.uk_scale[1]*kurt*np.ones((Nislice,dimY,dimX),dtype=DTYPE)
    ...
    G_x = self.execute_forward_3D(np.array([test_M0/self.uk_scale[0],ADC_x,ADC_xy
                                            ,ADC_y,ADC_xz
                                            ,ADC_z,ADC_yz,kurt],dtype=DTYPE))
...
#    self.uk_scale[0] = 2
    ...
    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0],ADC_x,ADC_xy
                                            ,ADC_y,ADC_xz
                                            ,ADC_z,ADC_yz,kurt],dtype=DTYPE))
    ...
    self.uk_scale[7] = self.uk_scale[7]*np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[7,...]))

    DG_x =  self.execute_gradient_3D(np.array([images[0]/self.uk_scale[0],ADC_x/self.uk_scale[1],ADC_xy/self.uk_scale[2],ADC_y/self.uk_scale[3],ADC_xz/self.uk_scale[4],ADC_z/self.uk_scale[5],ADC_yz/self.uk_scale[6],kurt/self.uk_scale[7]],dtype=DTYPE))
    ...
    print('Grad Scaling init', np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[7,...])))
    ...
    result = np.array([np.ones_like(test_M0)/self.uk_scale[0],
                       np.mean(ADC_x)*np.ones_like(ADC_x)/self.uk_scale[1],np.mean(ADC_xy)*np.ones_like(ADC_xy)/self.uk_scale[2],
                       np.mean(ADC_y)*np.ones_like(ADC_y)/self.uk_scale[3],np.mean(ADC_xz)*np.ones_like(ADC_xz)/self.uk_scale[4],
                       np.mean(ADC_z)*np.ones_like(ADC_z)/self.uk_scale[5],np.mean(ADC_yz)*np.ones_like(ADC_yz)/self.uk_scale[6],0*np.ones_like(kurt)/self.uk_scale[7]],dtype=DTYPE)
    self.guess = result
    ...
    self.constraints.append(constraint((-1e0/self.uk_scale[2]),(1e0/self.uk_scale[2]),True))
    self.constraints.append(constraint((0/self.uk_scale[3]),(5e0/self.uk_scale[3]),True))
    self.constraints.append(constraint((-1e0/self.uk_scale[4]),(1e0/self.uk_scale[4]),True))
    self.constraints.append(constraint((0/self.uk_scale[5]),(5e0/self.uk_scale[5]),True))
    self.constraints.append(constraint((-1e0/self.uk_scale[6]),(1e0/self.uk_scale[6]),True))
    self.constraints.append(constraint((-5/self.uk_scale[7]),(5/self.uk_scale[7]),True))
  def rescale(self,x):
    ...
    return np.array((M0,ADC_x,ADC_xy,ADC_y,ADC_xz,ADC_z,ADC_yz,kurt))
  ...
  def execute_forward_3D(self,x):
    ...
    ADC = ADC_x*self.dir[...,0]**2+ADC_y*self.dir[...,1]**2+ADC_z*self.dir[...,2]**2+\
          2*ADC_xy*self.dir[...,0]*self.dir[...,1]+2*ADC_xz*self.dir[...,0]*self.dir[...,2]+\
          2*ADC_yz*self.dir[...,1]*self.dir[...,2]
  ...
  def execute_gradient_3D(self,x):
    ...
    ADC = ADC_x*ADC_x_sc*self.dir[...,0]**2+ADC_y*ADC_y_sc*self.dir[...,1]**2+ADC_z*ADC_z_sc*self.dir[...,2]**2+\
          2*ADC_xy*ADC_xy_sc*self.dir[...,0]*self.dir[...,1]+2*ADC_xz*ADC_xz_sc*self.dir[...,0]*self.dir[...,2]+\
          2*ADC_yz*ADC_yz_sc*self.dir[...,1]*self.dir[...,2]


    meanADC = 1/3*(ADC_x*ADC_x_sc+ADC_y*ADC_y_sc+ADC_z*ADC_z_sc)
    kurt = kurt*kurt_sc
    ...
    grad_ADC_x = M0*M0_sc*(2/6*1/3*meanADC*ADC_x_sc*self.b**2*kurt-ADC_x_sc*self.dir[...,0]**2*self.b)*diffexp
    grad_ADC_xy= M0*M0_sc*(-2*ADC_xy_sc*self.dir[...,0]*self.dir[...,1]*self.b)*diffexp

    grad_ADC_y = M0*M0_sc*(2/6*1/3*meanADC*ADC_y_sc*self.b**2*kurt- ADC_y_sc*self.dir[...,1]**2*self.b)*diffexp
    grad_ADC_xz= M0*M0_sc*(-2*ADC_xz_sc*self.dir[...,0]*self.dir[...,2]*self.b)*diffexp

    grad_ADC_z = M0*M0_sc*(2/6*1/3*meanADC*ADC_z_sc*self.b**2*kurt - ADC_z_sc*self.dir[...,2]**2*self.b)*diffexp
    grad_ADC_yz= M0*M0_sc*(-2*ADC_yz_sc*self.dir[...,1]*self.dir[...,2]*self.b)*diffexp


    grad_kurt= M0*M0_sc*(1/6*meanADC**2*self.b**2*kurt_sc)*diffexp
#
  ...
  def plot_unknowns(self,x,dim_2D=False):
      ...
      if dim_2D:
         ...
         if not self.figure:
           ...
           self.kurt_plot=self.ax[61].imshow((kurt[int(self.NSlice/2),...]))
           self.kurt_plot_cor=self.ax[71].imshow((kurt[:,int(kurt.shape[1]/2),...]))
           self.kurt_plot_sag=self.ax[62].imshow(np.flip((kurt[:,:,int(kurt.shape[-1]/2)]).T,1))
