...
class VFA_Model:
  def __init__(self,fa,fa_corr,TR,images,Nislice,Nproj):
    ...
    test_T1 = 1/self.uk_sc[1]*np.exp(-self.TR/(test_T1*np.ones((Nislice,dimY,dimX),dtype=DTYPE)))


    G_x = self.execute_forward_3D(np.array([test_M0/self.uk_sc[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_T1],dtype=DTYPE))
    self.uk_sc[0] = self.uk_sc[0]*np.median(np.abs(images))/np.median(np.abs(G_x))

    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_sc[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_T1],dtype=DTYPE))
    self.uk_sc[1] = self.uk_sc[1]*np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[1,...]))

    self.uk_sc[1] = self.uk_sc[1]/np.sqrt(self.uk_sc[0])
    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_sc[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_T1],dtype=DTYPE))


    result = np.array([1/self.uk_sc[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),1/self.uk_sc[1]*np.exp(-self.TR/(800*np.ones((Nislice,dimY,dimX),dtype=DTYPE)))],dtype=DTYPE)
    self.guess = result
    self.constraints.append(constraint(-300,300,False)  )
    self.constraints.append(constraint(np.exp(-self.TR/(50))/self.uk_sc[1],np.exp(-self.TR/(5500))/self.uk_sc[1],True))

  def execute_forward_2D(self,x,islice):
    E1 = x[1,...]*self.uk_sc[1]
    S = x[0,:,:]*self.uk_sc[0]*(-E1 + 1)*self.sin_phi[:,islice,:,:]/(-E1*self.cos_phi[:,islice,:,:] + 1)
  ...
  def execute_gradient_2D(self,x,islice):
    ...
    M0 = x[0,...]
    E1[~np.isfinite(E1)] = 0
    grad_M0 = self.uk_sc[0]*(-E1 + 1)*self.sin_phi[:,islice,:,:]/(-E1*self.cos_phi[:,islice,:,:] + 1)
    grad_T1 = M0*self.uk_sc[0]*self.uk_sc[1]*(-E1 + 1)*self.sin_phi[:,islice,:,:]*self.cos_phi[:,islice,:,:]/(-E1*self.cos_phi[:,islice,:,:] + 1)**2 -\
    M0*self.uk_sc[0]*self.uk_sc[1]*self.sin_phi[:,islice,:,:]/(-E1*self.cos_phi[:,islice,:,:] + 1)
  ...
  def execute_forward_3D(self,x):
    E1 = x[1,...]*self.uk_sc[1]
    S = x[0,:,:]*self.uk_sc[0]*(-E1 + 1)*self.sin_phi/(-E1*self.cos_phi + 1)
    S[~np.isfinite(S)] = 1e-20
    S = np.array(S,dtype=DTYPE)
    return S
  def execute_gradient_3D(self,x):
    ...
    M0 = x[0,...]
    E1[~np.isfinite(E1)] = 0
    grad_M0 = self.uk_sc[0]*(-E1 + 1)*self.sin_phi/(-E1*self.cos_phi + 1)
    grad_T1 = M0*self.uk_sc[0]*self.uk_sc[1]*(-E1 + 1)*self.sin_phi*self.cos_phi/(-E1*self.cos_phi + 1)**2 -\
    M0*self.uk_sc[0]*self.uk_sc[1]*self.sin_phi/(-E1*self.cos_phi + 1)
    grad = np.array([grad_M0,grad_T1],dtype=DTYPE)
    grad[~np.isfinite(grad)] = 1e-20
    return grad
  ...
  def plot_unknowns(self,x,dim_2D=False):
      M0 = np.abs(x[0,...]*self.uk_sc[0])
      T1 = np.abs(-self.TR/np.log(x[1,...]*self.uk_sc[1]))
      M0_min = M0.min()
      M0_max = M0.max()
      T1_min = T1.min()
      T1_max = T1.max()
      ...
      if dim_2D:
       ...
       if not self.figure:
         ...
         self.M0_plot=self.ax[1].imshow((M0[int(self.Nislice/2),...].T))
         ...
         self.T1_plot=self.ax[3].imshow((T1[int(self.Nislice/2),...].T))
         ...
         self.M0_plot.set_data((M0[int(self.Nislice/2),...].T))
         ...
         self.T1_plot.set_data((T1[int(self.Nislice/2),...].T))
