...
#    cl.enqueue_copy(queue, ofs_buf, ofs.data).wait()
    ...
    sinogram_shape = (nd, self.Nproj,self.NC*self.NScan)
    
    return (ofs_buf, (self.dimY,self.dimX,self.NC*self.NScan), sinogram_shape)     
  ...
  def irgn_solve_2D(self, x, iters, data):
    ...
    a = np.reshape(a.get().T,(self.NScan,self.NC,self.Nproj,self.N))
...
#       self.cur_scan = j
    res = np.reshape(res,(self.NScan*self.NC,self.Nproj,self.N))
    (x,v, values) = self.tgv_radon(res.T, x.get()[:,None,...].T, (self.dimX,self.dimY,self.NSlice), self.Nproj, (2*alpha, alpha), iters,
                              record_values=True)
...
#            return
  ...
  def eval_fwd(self,y,x,wait_for=None):
    ...
    return self.prg.operator_fwd(y.queue, (self.dimY,self.dimX), None, 
                                 ...
                                 np.int32(self.NC), np.int32(1), np.int32(self.NScan), np.int32(self.unknowns),
                                 wait_for=wait_for)       
...
#    tmp_img = clarray.to_device(self.queue,np.require(np.reshape(x.get(),(self.unknowns*self.NSlice,self.dimY,self.dimX)),DTYPE,"C"))
    ...
    tmp_result = clarray.zeros(self.queue,(self.dimY,self.dimX,self.NC*self.NScan),DTYPE,"F")
...
#    return result
  ...
  def eval_adj(self,x,y,wait_for=None):
    ...
    return self.prg.operator_ad(x.queue, (self.dimY,self.dimX), None, 
                                 ...
                                 np.int32(1),  np.int32(self.NScan), np.int32(self.unknowns),
                                 wait_for=wait_for)      
...
#  
  def update_p(self,p, u, w, sigma, alpha, theta,wait_for=[]):
    p.add_event(self.prg.update_p(p.queue, u.shape[:-1], None, p.data, u.data,
                w.data, np.float32(sigma), np.float32(1.0/alpha), np.int32(self.unknowns), np.float32(theta),
                wait_for=u.events + w.events + p.events + wait_for))

  def update_q(self,q, w, sigma, alpha, theta,wait_for=[]):
    q.add_event(self.prg.update_q(q.queue, q.shape[1:-1], None, q.data, w.data,
                np.float32(sigma), np.float32(1.0/alpha),np.int32(self.unknowns), np.float32(theta),
...
## main iteration
  ...
  def tgv_radon(self,f0, x0, img_shape, angles, alpha, maxiter, record_values=True):
    ...
    w = clarray.zeros(self.queue, (2,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    w_ = clarray.zeros(self.queue, (2,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    p = clarray.zeros(self.queue, (2,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    p_ = clarray.zeros(self.queue, (2,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    q = clarray.zeros(self.queue, (4,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    q_ = clarray.zeros(self.queue, (4,self.dimX,self.dimY,self.NSlice,self.unknowns), dtype=DTYPE, order='F')
    ...
    L = ((0.5*(18.0 + np.sqrt(33)))+(L))
    print('L: %f'%(L))
    
    mu = 1/self.irgn_par.delta
    theta_line = 1.0
    beta_line = 400
    beta_new = 0
    mu_line = 0.5
...
#    L = 0.5*(18.0 + np.sqrt(33))
    ...
    for i in range(np.minimum(10,maxiter)):
        ...
        while True:
           ...
           self.update_p(p_, u, w, sigma, alpha[1],theta_line)
           self.update_q(q_, w, sigma, alpha[0], theta_line,wait_for=w_.events)
...
#        (sigma,tau) = self.step_update(u-u_,w-w_,sigma,tau,p_d_ratio);
    ...
    for i in np.arange(10,maxiter):
        self.update_p(p, u, w, sigma, alpha[1])
        self.update_q(q, w, sigma, alpha[0], wait_for=w_.events)
        Ku.add_event(self.operator_forward_2D(Ku, u, wait_for=u.events))
        self.update_lambda(lamb, Ku, f, sigma, normest)
        Kstarlambda.add_event(self.operator_adjoint_2D(Kstarlambda, lamb, wait_for=lamb.events))
        self.update_u(u_, u, p, Kstarlambda, uk, tau, normest, self.irgn_par.delta)
        ...
        if (i % 50 == 0):
            ...
            (sigma,tau) = self.step_update(u_-u,w_-w,sigma,tau,p_d_ratio);
                        

    if record_values:
        sys.stdout.write('\n')
        return (np.squeeze(u.get().T), np.squeeze(w.get().T), values)
    else:
        return u.get()
...
#       for j in range(self.NC):
    tmp_img = clarray.to_device(self.queue,np.require(np.reshape(x,(self.NC*self.NScan,self.dimY,self.dimX)),DTYPE,"C"))
