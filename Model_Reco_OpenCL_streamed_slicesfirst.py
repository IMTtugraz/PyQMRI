...
#### Allocate temporary Arrays
    ...
    for j in range(2*self.num_dev):
      Axold_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
      Kyk1_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
      Kyk2_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
...
#### Stream
    for j in range(2*self.num_dev,int(self.NSlice/(2*self.par_slices*self.num_dev)+(2*self.num_dev-1))):
        for i in range(self.num_dev):
          ...
          z2_part[i]=(clarray.to_device(self.queue[3*i], z2[idx_start:idx_stop,...],allocator=self.alloc[i]))# ))
        ...
        for i in range(self.num_dev):
          ...
          z2_part[self.num_dev+i]=(clarray.to_device(self.queue[3*i+1], z2[idx_start:idx_stop,...] ,allocator=self.alloc[i]))# ))
...
#### Collect last block
  ...
  #### Allocate temporary Arrays
      if myit == 0:
        ...
        for j in range(2*self.num_dev):
          x_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          Ax_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          v_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          gradx_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          gradx_xold_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          symgrad_v_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
          symgrad_v_vold_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
  ...
  #### Collect last block
      ...
      while True:
        ...
        if myit == 0:
          ...
          for j in range(2*self.num_dev):
            z1_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
            z2_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,8),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
            r_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
            Kyk1_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
            Kyk2_new_part.append(clarray.zeros(self.queue[3*int(np.mod(j,self.num_dev))],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX,4),dtype=DTYPE,allocator=self.alloc[int(np.mod(j,self.num_dev))]))
...
#          tic = time.time()
          for i in range(self.num_dev):
            ...
            ynorm += ((clarray.vdot(r_new_part[i][:(self.par_slices),...]-r_part[i][:(self.par_slices),...],r_new_part[i][:(self.par_slices),...]-r_part[i][:(self.par_slices),...],queue=self.queue[3*i])+clarray.vdot(z1_new_part[i][:(self.par_slices),...]-z1_part[i][:(self.par_slices),...],z1_new_part[i][:(self.par_slices),...]-z1_part[i][:(self.par_slices),...],queue=self.queue[3*i])+clarray.vdot(z2_new_part[i][:(self.par_slices),...]-z2_part[i][:(self.par_slices),...],z2_new_part[i][:(self.par_slices),...]-z2_part[i][:(self.par_slices),...],queue=self.queue[3*i]))).get()
            lhs += ((clarray.vdot(Kyk1_new_part[i][:(self.par_slices),...]-Kyk1_part[i][:(self.par_slices),...],Kyk1_new_part[i][:(self.par_slices),...]-Kyk1_part[i][:(self.par_slices),...],queue=self.queue[3*i])+clarray.vdot(Kyk2_new_part[i][:(self.par_slices),...]-Kyk2_part[i][:(self.par_slices),...],Kyk2_new_part[i][:(self.par_slices),...]-Kyk2_part[i][:(self.par_slices),...],queue=self.queue[3*i]))).get()
            ### Put Data
            idx_start = i*self.par_slices+(2*self.num_dev*(j-2*self.num_dev+1)*self.par_slices)-self.overlap
            idx_stop = (i+1)*self.par_slices+(2*self.num_dev*(j-2*self.num_dev+1))*self.par_slices
            z1_part[i].set(  z1[idx_start:idx_stop,...] ,self.queue[3*i])
          ...
          for i in range(self.num_dev):
            ...
            Kyk1_new[idx_start:idx_stop,...] = Kyk1_new_tmp[self.overlap:,...]
            Kyk2_new[idx_start:idx_stop,...] = Kyk2_new_tmp[self.overlap:,...]
            ynorm += ((clarray.vdot(r_new_part[i+self.num_dev][:(self.par_slices),...]-r_part[i+self.num_dev][:(self.par_slices),...],r_new_part[i+self.num_dev][:(self.par_slices),...]-r_part[i+self.num_dev][:(self.par_slices),...],queue=self.queue[3*i+1])+clarray.vdot(z1_new_part[i+self.num_dev][:(self.par_slices),...]-z1_part[i+self.num_dev][:(self.par_slices),...],z1_new_part[i+self.num_dev][:(self.par_slices),...]-z1_part[i+self.num_dev][:(self.par_slices),...],queue=self.queue[3*i+1])+clarray.vdot(z2_new_part[i+self.num_dev][:(self.par_slices),...]-z2_part[i+self.num_dev][:(self.par_slices),...],z2_new_part[i+self.num_dev][:(self.par_slices),...]-z2_part[i+self.num_dev][:(self.par_slices),...],queue=self.queue[3*i+1]))).get()
            lhs += ((clarray.vdot(Kyk1_new_part[i+self.num_dev][:(self.par_slices),...]-Kyk1_part[i+self.num_dev][:(self.par_slices),...],Kyk1_new_part[i+self.num_dev][:(self.par_slices),...]-Kyk1_part[i+self.num_dev][:(self.par_slices),...],queue=self.queue[3*i+1])+clarray.vdot(Kyk2_new_part[i+self.num_dev][:(self.par_slices),...]-Kyk2_part[i+self.num_dev][:(self.par_slices),...],Kyk2_new_part[i+self.num_dev][:(self.par_slices),...]-Kyk2_part[i+self.num_dev][:(self.par_slices),...],queue=self.queue[3*i+1]))).get()
            ### Put Data
            idx_start = i*self.par_slices+(2*self.num_dev*(j-2*self.num_dev+1)+self.num_dev)*self.par_slices-self.overlap
            idx_stop = (i+1)*self.par_slices+(2*self.num_dev*(j-2*self.num_dev+1)+self.num_dev)*self.par_slices
            z1_part[i+self.num_dev].set( z1[idx_start:idx_stop,...] ,self.queue[3*i+1])
...
#          return x
  ...
  def FT_streamed(self,outp,inp):
      cl_out = []
      for j in range(self.num_dev):
        cl_out.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_out.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
      cl_data = []
      for i in range(self.num_dev):
        idx_start = i*self.par_slices
        idx_stop = (i+1)*self.par_slices
  ...
  def operator_forward_streamed(self, outp, inp):
      ...
      for j in range(self.num_dev):
        cl_out.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_out.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.Nproj,self.N),dtype=DTYPE))
        cl_tmp.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
        cl_tmp.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
...
#      par_slices = self.par_slices
      for j in range(self.num_dev):
        cl_out.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
        cl_out.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
      cl_data = []
      for i in range(self.num_dev):
...
#        print("Get Start: %i, Stop: %i" %(i*par_slices+(2*self.num_dev*(j-2*self.num_dev)+self.num_dev)*par_slices,(i+1)*par_slices+(2*self.num_dev*(j-2*self.num_dev)+self.num_dev)*par_slices))
  ...
  def operator_adjoint_streamed(self, outp, inp):
      ...
      for j in range(self.num_dev):
        cl_out.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE))
        cl_out.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.unknowns,self.dimY,self.dimX),dtype=DTYPE))
        cl_tmp.append(clarray.zeros(self.queue[3*j],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
        cl_tmp.append(clarray.zeros(self.queue[3*j+1],(self.par_slices+self.overlap,self.NScan,self.NC,self.dimY,self.dimX),dtype=DTYPE))
