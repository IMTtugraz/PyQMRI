...
#
    G_x = self.execute_forward_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1,ADC2],dtype=DTYPE))
    self.uk_scale[0] = self.uk_scale[0]*np.max(np.abs(images))/np.median(np.abs(G_x))

    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1,ADC2],dtype=DTYPE))
    ...
    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1/self.uk_scale[2],ADC2/self.uk_scale[3]],dtype=DTYPE))
#    print('Grad Scaling init', np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[1,...])))
    ...
    result = np.array([1/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),0.5/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),(1e-4/self.uk_scale[2]*np.ones((Nislice,dimY,dimX),dtype=DTYPE)),(1e-2/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE))],dtype=DTYPE)
    self.guess = result
    ...
    self.constraints.append(constraint((1e-5/self.uk_scale[2]),((1/1e-3)/self.uk_scale[2]),True))
    self.constraints.append(constraint((1e-3/self.uk_scale[3]),((1e-1)/self.uk_scale[3]),True))
  def rescale(self,x):
    M0 = x[0,...]*self.uk_scale[0]
    M01 = x[1,...]*self.uk_scale[1]
    ADC1 = 1/(x[2,...]*self.uk_scale[2])
    ADC2 = 1/(x[3,...]*self.uk_scale[3])
  ...
  def execute_forward_2D(self,x,islice):
    ...
    S = M0*(M01*np.exp(-self.TE*(ADC1))+(1-M01)*np.exp(-self.TE*(ADC2)))
  ...
  def execute_gradient_2D(self,x,islice):
    ...
    ADC1 = x[2,...]
    ADC2 = x[3,...]
    grad_M0 =  self.uk_scale[0]*(M01*self.uk_scale[1]*np.exp(-self.TE*(ADC1*self.uk_scale[2])) + (1-M01*self.uk_scale[1])*np.exp(-self.TE*(ADC2*self.uk_scale[3])))
    grad_M01 = self.uk_scale[0]*M0*(self.uk_scale[1]*np.exp(-self.TE*(ADC1*self.uk_scale[2]))-self.uk_scale[1]*np.exp(-self.TE*(ADC2*self.uk_scale[3])))
    grad_ADC1 = -self.uk_scale[0]*M0*M01*self.uk_scale[1]*self.TE*self.uk_scale[2]*np.exp(-self.TE*(ADC1*self.uk_scale[2]))
    grad_ADC2 = -self.uk_scale[0]*M0*(1-M01*self.uk_scale[1])*self.TE*self.uk_scale[3]*np.exp(-self.TE*(ADC2*self.uk_scale[3]))
