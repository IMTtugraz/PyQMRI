...
for files in filenames:
    ...
    if "ref" in files:
        ...
        plot_names.append(fname[-5:].split('_')[0] +
                          " s " + fname[-5:].split('_')[1] + " TGV")
        plot_names.append(fname[-5:].split('_')[0] +
                          " s " + fname[-5:].split('_')[1] + " Tikh")
...
mask = mask[0, int(y / 2) - half_im_size:int(y / 2) + half_im_size,
            int(x / 2) - half_im_size:int(x / 2) + half_im_size]
...
if "Reference" in plot_names:
    ...
    T1_plot.append((
        T1_ref[pos,
               int(dimy / 2) - half_im_size:int(dimy / 2) + half_im_size,
               int(dimx / 2) - half_im_size:int(dimx / 2) + half_im_size]
        * mask).T)
...
for i in range(NResults - NRef):
    ...
    T1_tgv[i] = T1_tgv[i][pos,
                          int(y / 2) - half_im_size:int(y / 2) + half_im_size,
                          int(x / 2) - half_im_size:int(x / 2) + half_im_size] * mask
    M0_tgv[i] = M0_tgv[i][pos,
                          int(y / 2) - half_im_size:int(y / 2) + half_im_size,
                          int(x / 2) - half_im_size:int(x / 2) + half_im_size] * mask
    T1_tikh[i] = T1_tikh[i][pos,
                            int(y / 2) - half_im_size:int(y / 2) + half_im_size,
                            int(x / 2) - half_im_size:int(x / 2) + half_im_size] * mask
    M0_tikh[i] = M0_tikh[i][pos,
                            int(y / 2) - half_im_size:int(y / 2) + half_im_size,
                            int(x / 2) - half_im_size:int(x / 2) + half_im_size] * mask
...
#  M0_plot.append(np.squeeze(M0_tikh[i][...]).T)
    ...
    if "Reference" in plot_names:
        T1_err.append(np.squeeze(np.abs(T1_tgv[i] - T1_ref[-1,
                                                           int(dimy / 2) - half_im_size:int(dimy / 2) + half_im_size,
                                                           int(dimx / 2) - half_im_size:int(dimx / 2) + half_im_size]) / np.abs(T1_ref[-1,
                                                                                                                                       int(dimy / 2) - half_im_size:int(dimy / 2) + half_im_size,
                                                                                                                                       int(dimx / 2) - half_im_size:int(dimx / 2) + half_im_size])).T * 100)
        T1_err.append(np.squeeze(np.abs(T1_tikh[i] - T1_ref[-1,
                                                            int(dimy / 2) - half_im_size:int(dimy / 2) + half_im_size,
                                                            int(dimx / 2) - half_im_size:int(dimx / 2) + half_im_size]) / np.abs(T1_ref[-1,
                                                                                                                                        int(dimy / 2) - half_im_size:int(dimy / 2) + half_im_size,
                                                                                                                                        int(dimx / 2) - half_im_size:int(dimx / 2) + half_im_size])).T * 100)
...
if "Reference" in plot_names and plot_err:
    ...
    for i in range((NResults)):
        ...
        im = ax[i].imshow(np.abs(T1_plot[2 * i]), vmin=T1_min,
                          vmax=T1_max, aspect=1, cmap=cm.viridis)
        ax[i].axis('off')
    #  ax[i].set_anchor("N")
    ...
    for i in range((NResults)):
        ...
        im = ax[i + NResults].imshow(np.abs(T1_plot[2 * i + 1]),
                                     vmin=T1_min, vmax=T1_max, aspect=1, cmap=cm.viridis)
    ...
    for i in range((NResults - NRef)):
        ...
        im = ax[i + 2 * (NResults)].imshow(np.abs(T1_err[2 * i]),
                                           vmin=T1_err_min, vmax=T1_err_max, aspect=1, cmap=cm.inferno)
        ax[i + 2 * (NResults)].axis('off')
        ax[i + 2 * (NResults)].set_anchor("N")
        ax[i + 2 * (NResults)].set_title(plot_names[2 * (i + 1)],
                                         color='white')

    for i in range((NResults - NRef)):
        ax.append(plt.subplot(gs[i + 4 * (NResults + 1) + 1]))
        im = ax[i + 3 * (NResults) - 1].imshow(np.abs(T1_err[2 * i + 1]),
                                               vmin=T1_err_min, vmax=T1_err_max, aspect=1, cmap=cm.inferno)
        ax[i + 3 * (NResults) - 1].axis('off')
        ax[i + 3 * (NResults) - 1].set_anchor("S")
        ax[i + 3 * (NResults) - 1].set_title(plot_names[2 *
                                                        (i + 1) + 1], color='white')
...
if roi_num > 0:
    if "Reference" not in plot_names:
        T1_ref = T1_tgv[0]
    mean_TGV = []
    mean_Tikh = []
    std_TGV = []
    std_Tikh = []
    col_names = []
    selector = cv2.cvtColor(np.abs(np.squeeze(
        T1_ref[0, ...]).T / 3000).astype(np.float32), cv2.COLOR_GRAY2BGR)

    for j in range(roi_num):
        r = (cv2.selectROI(selector, False))
        col_names.append("ROI " + str(j + 1))
        mean_TGV.append(np.abs(
            np.mean(T1_ref[0, int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
        std_TGV.append(np.abs(
            np.std(T1_ref[0, int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
        for i in range((NResults - 1)):
            mean_TGV.append(np.abs(
                np.mean(T1_tgv[i][int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
            mean_Tikh.append(np.abs(
                np.mean(T1_tikh[i][int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
            std_TGV.append(np.abs(
                np.std(T1_tgv[i][int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
            std_Tikh.append(np.abs(
                np.std(T1_tikh[i][int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
        rects = patches.Rectangle(
            (int(
                r[0]), int(
                r[1])), int(
                r[2]), int(
                    r[3]), linewidth=1, edgecolor='r', facecolor='none')
        posx = int(r[1] - 5)
        posy = int(r[0])
        ax[0].text(posy, posx, str(j + 1), color='red')
        ax[0].add_patch(rects)

    mean_TGV = np.round(pd.DataFrame(np.reshape(np.asarray(
        mean_TGV), (roi_num, NResults)).T, index=plot_names[0::2], columns=col_names), decimals=0)
    mean_Tikh = np.round(pd.DataFrame(np.reshape(np.asarray(
        mean_Tikh), (roi_num, NResults - 1)).T, index=plot_names[3::2], columns=col_names), decimals=0)
    std_TGV = np.round(pd.DataFrame(np.reshape(np.asarray(
        std_TGV), (roi_num, NResults)).T, index=plot_names[0::2], columns=col_names), decimals=0)
    std_Tikh = np.round(pd.DataFrame(np.reshape(np.asarray(std_Tikh),
                                                (roi_num,
                                                 NResults - 1)).T,
                                     index=plot_names[3::2],
                                     columns=col_names),
                        decimals=0)
...
plt.savefig('/media/data/Papers/Parameter_Mapping/2Dvs3D_' +
            save_name + '.eps', format='eps', dpi=1000)
