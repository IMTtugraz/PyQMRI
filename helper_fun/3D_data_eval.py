...
import helper_fun.multislice_viewer as msv
...
#  extent=[0,plot_extend_x[i],0,plot_extend_y[i]]
    ...
    for i in range(6):
        ...
        if i == 2 or i == 5:
            ...
            im = ax[i + j * 6].imshow(np.abs(T1_plot[i]),
                                      vmin=T1_min,
                                      vmax=T1_max,
                                      aspect=1,
                                      cmap=cm.viridis)
...
plt.savefig(
    '/media/omaier/a3c6e764-0f9b-44b3-b888-26da7d3fe6e7/Papers/Parameter_Mapping/3D_' +
    save_name +
    '.eps',
    format='eps',
    dpi=1000)
...
if roi_num > 0:
    ...
    std_TGV = []
    col_names = []
    selector = cv2.cvtColor(np.abs(T1_ref[int(
        z / 2), :, :, ].T / np.max(T1_ref)).astype(np.float32), cv2.COLOR_GRAY2BGR)
    for j in range(roi_num):
        r = (cv2.selectROI(selector))
        col_names.append("ROI " + str(j + 1))
        mean_TGV.append(np.abs(np.mean(
            T1_ref[int(z / 2), int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
        std_TGV.append(np.abs(np.std(
            T1_ref[int(z / 2), int(r[0]):int(r[0] + r[2]), int(r[1]):int(r[1] + r[3])])))
        for i in range(NResults):
            mean_TGV.append(np.abs(np.mean(T1_tgv[i][int(z / 2), int(r[0] + offset):int(
                r[0] + r[2] + offset), int(r[1]) + offset:int(r[1] + r[3] + offset)])))
            std_TGV.append(np.abs(np.std(T1_tgv[i][int(z / 2),
                                                   int(r[0] + offset):int(r[0] + r[2] + offset),
                                                   int(r[1] + offset):int(r[1] + r[3] + offset)])))
        rects = patches.Rectangle(
            (int(
                r[0]), int(
                r[1])), int(
                r[2]), int(
                    r[3]), linewidth=3, edgecolor='r', facecolor='none')
        posx = int(r[0])
        posy = int(r[1] - 5)
