...
def optimizedPattern(data, AF, ACL):
    ...
    [dimY, dimX, NScan, NCoil, NAlpha] = data.T.shape
    ...
    for k in range(0, NAlpha):
        ...
        mask = np.zeros([test.shape[0] - 1, test.shape[0] - 1],
                        dtype=np.complex64)  # size noch festlegen
        ...
        mask = mask[int(np.floor(yCut / 2)):int(np.floor(yMask - yCut / 2)),
                    int(np.floor(xCut / 2)):int(np.floor(xMask - xCut / 2))]
