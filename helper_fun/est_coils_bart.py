    ...
    try:
        if not file['Coils'][()].shape[1] >= par["NSlice"] and not args.sms:
            if args.trafo:
                ...
                par["C"] = np.zeros(
                    (par["NC"], par["NSlice"], par["dimY"], par["dimX"]), dtype=DTYPE)
...
#        par["C"] = np.transpose(par["C"],(0,2,3,1))
            ...
            if args.sms:
                ...
                par["C"] = file['Coils'][:, int(slices_coils / 2) - int(np.floor(
