import numpy as np
import helper_fun.phantom as pt
...
def FLASH(M0f, T1f, alphaf, TRf):
    ...
    np.divide(M0f * sin(alphaf) * (1 - np.exp(-np.divide(TRf, T1f))),
              (1 - cos(alphaf) * np.exp(-np.divide(TRf, T1f))), result)
...
# Taken from
# https://github.com/ismrmrd/ismrmrd-python-tools/blob/master/ismrmrdtools/simulation.py
...
def generate_birdcage_sensitivities(
    ...
    for c in range(0, number_of_coils):
        ...
        for y in range(0, matrix_size):
            ...
            for x in range(0, matrix_size):
                x_co = float(x - matrix_size / 2) / \
                    float(matrix_size / 2) - coilx
    ...
    if normalize:
        ...
        out = out / np.repeat(rss.reshape(matrix_size,
                                          matrix_size, 1), number_of_coils, axis=2)
...
def generate_phantom_data(N=128, M=128, NC=4, TR=5, TR_bSSFP=5, genBoth=False):
    ...
    for i in range(0, np.size(alpha)):
        data1[:, :, :, i] = (np.multiply(np.repeat(
            FLASH(M0, T1, alpha[i], TR) + commnoise, NC, axis=2), (coils_noisy)))
    if (genBoth):
        alpha_bSSFP = np.array(
            [5, 10, 15, 20, 25, 30, 35, 40, 50, 60, 70]) * pi / 180
        # Add Noise to every coil:
        ...
        for i in range(0, np.size(alpha_bSSFP)):
            data2[:,
                  :,
                  :,
                  i] = np.fft.fft2(np.multiply(np.repeat(bSSFP(M0,
                                                               T1,
                                                               T2,
                                                               alpha_bSSFP[i],
                                                               TR_bSSFP) + commnoise,
