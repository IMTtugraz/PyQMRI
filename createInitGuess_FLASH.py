...
def createInitGuess_FLASH(dat,FA,TR):
  ...
  [numAlpha, dimX, dimY] = imgs.shape
  
  #    alpha = np.copy(FA)
  DESPOT1    = np.zeros([dimX,dimY],dtype='complex128')#np.complex128
  M0         = np.zeros([dimX,dimY],dtype='complex128')
  mask_guess = np.zeros([dimX,dimY],dtype='complex128')
  imgsize = dimX*dimY
...
#            Y = imgs[i,iSlice,:,:] * mask[MaskToUse,:,:] / np.sin(alphaCor[i,:,:])               
    X = imgs[i,:,:] / np.tan(FA[i,:,:])
    Y = imgs[i,:,:]  / np.sin(FA[i,:,:])        
...
####mit np.linalg.lstsq?
  ...
  b_tv = sol_regtv[1,:,:] * Y_std_tmp + Y_mean_tmp
   # print(m_tv)
  DESPOT1 = -TR / np.log(abs(m_tv))  #neg wird später berücksichtigt
