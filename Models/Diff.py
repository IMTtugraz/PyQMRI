...
#    self.guess = result
    ...
    def _execute_gradient_2D(self, x, islice):
        ...
        grad_M0 = self.uk_scale[0] * \
            np.exp(-self.TE * (ADC * self.uk_scale[1]))
        grad_ADC = -M0 * self.uk_scale[0] * self.TE * \
            self.uk_scale[1] * np.exp(-self.TE * (ADC * self.uk_scale[1]))
    ...
    def _execute_gradient_3D(self, x):
        M0 = x[0, ...]
        ADC = x[1, ...]
        grad_M0 = np.exp(-self.TE *
                         (ADC * self.uk_scale[1])) * self.uk_scale[0]
        grad_ADC = -M0 * self.uk_scale[0] * self.TE * \
            self.uk_scale[1] * np.exp(-self.TE * (ADC * self.uk_scale[1]))
        grad = np.array([grad_M0, grad_ADC], dtype=DTYPE)
        grad[~np.isfinite(grad)] = 1e-20
