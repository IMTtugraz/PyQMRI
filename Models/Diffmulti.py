...
class Model:
  def __init__(self,par,images):
    ...
    ADC1 = np.reshape(np.linspace(1e-4,1e-2,dimX*dimY*Nislice),(Nislice,dimX,dimY))
    test_M01 = 1
    ADC1 = 1/self.uk_scale[2]*ADC1*np.ones((Nislice,dimY,dimX),dtype=DTYPE)
    ADC2 = np.reshape(np.linspace(1e-4,1e-2,dimX*dimY*Nislice),(Nislice,dimX,dimY))
    test_M02 = 1
    ADC2 = 1/self.uk_scale[4]*ADC2*np.ones((Nislice,dimY,dimX),dtype=DTYPE)
#
    G_x = self.execute_forward_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1,test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC2],dtype=DTYPE))
    self.uk_scale[0] = self.uk_scale[0]*np.max(np.abs(images))/np.median(np.abs(G_x))

    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1,test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC2],dtype=DTYPE))
    ...
    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC1/self.uk_scale[2],test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),ADC2/self.uk_scale[4]],dtype=DTYPE))
#    print('Grad Scaling init', np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[1,...])))
    ...
    result = np.array([0.1/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),0.5/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),(1e-4/self.uk_scale[2]*np.ones((Nislice,dimY,dimX),dtype=DTYPE)),0.5/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),(1e-3/self.uk_scale[4]*np.ones((Nislice,dimY,dimX),dtype=DTYPE))],dtype=DTYPE)
    self.guess = result
    ...
    self.constraints.append(constraint((1e-4/self.uk_scale[2]),((1/1e-2)/self.uk_scale[2]),True))
    self.constraints.append(constraint(0/self.uk_scale[3],1/self.uk_scale[3],True)  )
    self.constraints.append(constraint((1e-4/self.uk_scale[4]),((1/1e-2)/self.uk_scale[4]),True))
  def rescale(self,x):
  ...
  def execute_forward_2D(self,x,islice):
    ...
    M02 = x[3,...]*self.uk_scale[3]
    ADC2 = x[4,...]*self.uk_scale[4]
    S = M0*(M01*np.exp(-self.TE*(ADC1))+M02*np.exp(-self.TE*(ADC2)))
  ...
  def execute_gradient_2D(self,x,islice):
    ...
    M02 = x[3,...]
    ADC2 = x[4,...]
    grad_M0 =  self.uk_scale[0]*(M01*self.uk_scale[1]*np.exp(-self.TE*(ADC1*self.uk_scale[2])) + M02*self.uk_scale[3]*np.exp(-self.TE*(ADC2*self.uk_scale[4])))
    grad_M01 = self.uk_scale[0]*M0*self.uk_scale[1]*np.exp(-self.TE*(ADC1*self.uk_scale[2]))
    grad_ADC1 = -self.uk_scale[0]*M0*M01*self.uk_scale[1]*self.TE*self.uk_scale[2]*np.exp(-self.TE*(ADC1*self.uk_scale[2]))
    grad_M02 = self.uk_scale[0]*M0*self.uk_scale[3]*np.exp(-self.TE*(ADC2*self.uk_scale[4]))
    grad_ADC2 = -self.uk_scale[0]*M0*M02*self.uk_scale[3]*self.TE*self.uk_scale[4]*np.exp(-self.TE*(ADC2*self.uk_scale[4]))
    grad = np.array([grad_M0,grad_M01,grad_ADC1,grad_M02,grad_ADC2],dtype=DTYPE)
  ...
  def plot_unknowns(self,x,dim_2D=False):
      ...
      if dim_2D:
         ...
         if not self.figure:
           ...
           self.gs = gridspec.GridSpec(2,17, width_ratios=[x/z,1,x/(20*z),x/z*0.25,x/(20*z),x/z,1,x/z,1,x/(20*z),x/z*0.25,x/(20*z),x/z,1,x/z,1,x/(20*z)],height_ratios=[x/z,1])
           self.figure.tight_layout()
           ...
           self.M0_plot=self.ax[0].imshow((M0[int(self.NSlice/2),...]))
           self.M0_plot_cor=self.ax[17].imshow((M0[:,int(M01.shape[1]/2),...]))
           self.M0_plot_sag=self.ax[1].imshow(np.flip((M0[:,:,int(M01.shape[-1]/2)]).T,1))
...
#           cax.yaxis.set_ticks_position('left')
           ...
           self.M01_plot=self.ax[5].imshow((M01[int(self.NSlice/2),...]))
           self.M01_plot_cor=self.ax[22].imshow((M01[:,int(M01.shape[1]/2),...]))
           self.M01_plot_sag=self.ax[6].imshow(np.flip((M01[:,:,int(M01.shape[-1]/2)]).T,1))
           ...
           self.M02_plot=self.ax[12].imshow((M02[int(self.NSlice/2),...]))
           self.M02_plot_cor=self.ax[29].imshow((M02[:,int(M02.shape[1]/2),...]))
           self.M02_plot_sag=self.ax[13].imshow(np.flip((M02[:,:,int(M02.shape[-1]/2)]).T,1))
           ...
           self.ADC1_plot=self.ax[7].imshow((ADC1[int(self.NSlice/2),...]))
           self.ADC1_plot_cor=self.ax[24].imshow((ADC1[:,int(ADC1.shape[1]/2),...]))
           self.ADC1_plot_sag=self.ax[8].imshow(np.flip((ADC1[:,:,int(ADC1.shape[-1]/2)]).T,1))
           ...
           self.ADC2_plot=self.ax[14].imshow((ADC2[int(self.NSlice/2),...]))
           self.ADC2_plot_cor=self.ax[31].imshow((ADC2[:,int(ADC2.shape[1]/2),...]))
           self.ADC2_plot_sag=self.ax[15].imshow(np.flip((ADC2[:,:,int(ADC2.shape[-1]/2)]).T,1))
           ...
           self.M0_plot_sag.set_data(np.flip((M0[:,:,int(M01.shape[-1]/2)]).T,1))
           ...
           self.ADC1_plot_cor.set_data((ADC1[:,int(ADC1.shape[1]/2),...]))
           self.ADC1_plot_sag.set_data(np.flip((ADC1[:,:,int(ADC1.shape[-1]/2)]).T,1))
           self.ADC1_plot.set_clim([ADC1_min,ADC1_max])
           ...
           self.M02_plot_cor.set_data((M02[:,int(M02.shape[1]/2),...]))
           self.M02_plot_sag.set_data(np.flip((M02[:,:,int(M02.shape[-1]/2)]).T,1))
           ...
           self.ADC2_plot_cor.set_data((ADC2[:,int(ADC2.shape[1]/2),...]))
           self.ADC2_plot_sag.set_data(np.flip((ADC2[:,:,int(ADC2.shape[-1]/2)]).T,1))
