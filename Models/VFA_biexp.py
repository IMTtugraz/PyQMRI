...
class Model:
  def __init__(self,par,images):
    ...
    test_M02 = 1
    T22 = 1/self.uk_scale[4]*T22*np.ones((Nislice,dimY,dimX),dtype=DTYPE)
#
    G_x = self.execute_forward_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T21,test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T22],dtype=DTYPE))
    self.uk_scale[0] = self.uk_scale[0]*np.max(np.abs(images))/np.median(np.abs(G_x))

    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T21,test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T22],dtype=DTYPE))
    ...
    DG_x =  self.execute_gradient_3D(np.array([test_M0/self.uk_scale[0]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),test_M01/self.uk_scale[1]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T21/self.uk_scale[2],test_M02/self.uk_scale[3]*np.ones((Nislice,dimY,dimX),dtype=DTYPE),T22/self.uk_scale[4]],dtype=DTYPE))
#    print('Grad Scaling init', np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[1,...])))
    ...
    self.constraints.append(constraint(((1/1500)/self.uk_scale[4]),((1/150)/self.uk_scale[4]),True))
  def rescale(self,x):
  ...
  def execute_forward_2D(self,x,islice):
    ...
    S = M0*(M01*np.exp(-self.TE*(T21))+M02*np.exp(-self.TE*(T22)))
  ...
  def execute_gradient_2D(self,x,islice):
    ...
    M02 = x[3,...]
    T22 = x[4,...]
    grad_M0 =  self.uk_scale[0]*(M01*self.uk_scale[1]*np.exp(-self.TE*(T21*self.uk_scale[2])) + M02*self.uk_scale[3]*np.exp(-self.TE*(T22*self.uk_scale[4])))
    grad_M01 = self.uk_scale[0]*M0*self.uk_scale[1]*np.exp(-self.TE*(T21*self.uk_scale[2]))
    grad_T21 = -self.uk_scale[0]*M0*M01*self.uk_scale[1]*self.TE*self.uk_scale[2]*np.exp(-self.TE*(T21*self.uk_scale[2]))
    grad_M02 = self.uk_scale[0]*M0*self.uk_scale[3]*np.exp(-self.TE*(T22*self.uk_scale[4]))
    grad_T22 = -self.uk_scale[0]*M0*M02*self.uk_scale[3]*self.TE*self.uk_scale[4]*np.exp(-self.TE*(T22*self.uk_scale[4]))
    grad = np.array([grad_M0,grad_M01,grad_T21,grad_M02,grad_T22],dtype=DTYPE)
...
#           cax.yaxis.set_ticks_position('left')
           ...
           self.T21_plot=self.ax[7].imshow((T21[int(self.NSlice/2),...]))
           self.T21_plot_cor=self.ax[24].imshow((T21[:,int(T21.shape[1]/2),...]))
           self.T21_plot_sag=self.ax[8].imshow(np.flip((T21[:,:,int(T21.shape[-1]/2)]).T,1))
           ...
           self.T22_plot=self.ax[14].imshow((T22[int(self.NSlice/2),...]))
           self.T22_plot_cor=self.ax[31].imshow((T22[:,int(T22.shape[1]/2),...]))
           self.T22_plot_sag=self.ax[15].imshow(np.flip((T22[:,:,int(T22.shape[-1]/2)]).T,1))
