...
#               (2*x[1,:,:]**2*self.T1_sc*(E1-self.cos_phi[:,slice,:,:])**2))
    grad_M0 = self.M0_sc*(-E1 + 1)*np.sin(fa*fa_corr)/(-E1*np.cos(fa*fa_corr) + 1)
    grad_T1 = x[0,...]*self.M0_sc*(-E1 + 1)*np.sin(fa*fa_corr)*np.cos(fa*fa_corr)/(-E1*np.cos(fa*fa_corr) + 1)**2 \
              - x[0,...]*self.M0_sc*np.sin(fa*fa_corr)/(-E1*np.cos(fa*fa_corr) + 1)
    grad_FA = -E1*x[0,...]*self.M0_sc*fa*(-E1 + 1)*np.sin(fa*fa_corr)**2/(-E1*np.cos(fa*fa_corr) + 1)**2 \
              + x[0,...]*self.M0_sc*fa*(-E1 + 1)*np.cos(fa*fa_corr)/(-E1*np.cos(fa*fa_corr) + 1)
