  ...
  cpdef execute_2D(self, int TV=0):
    ...
    if TV==1:
      for islice in range(self.par.NSlice):
        ...
        scale = self.T1_sc/self.model.uk_sc[1]
        self.model.uk_sc[1] *= scale
        ...
        for i in range(self.irgn_par["max_GN_it"]):
          ...
          for uk in range(self.unknowns-1):
            ...
            result[uk+1,islice,...] = result[uk+1,islice,...]*self.model.uk_sc[uk+1]
            self.model.uk_sc[uk+1] = self.model.uk_sc[uk+1]*scale
            result[uk+1,islice,...] = result[uk+1,islice,...]/self.model.uk_sc[uk+1]
          ...
          for uk in range(self.unknowns):
            self.result[i+1,uk,islice,...] = result[uk,islice,...]*self.model.uk_sc[uk]
   ...
   if TV==1:
      for i in range(self.irgn_par["max_GN_it"]):
        ...
        for uk in range(self.unknowns-1):
          ...
          self.model.constraints[uk+1].update(scale)
          result[uk+1,...] = result[uk+1,...]*self.model.uk_sc[uk+1]
          self.model.uk_sc[uk+1] = self.model.uk_sc[uk+1]*scale
          result[uk+1,...] = result[uk+1,...]/self.model.uk_sc[uk+1]
        ...
        for uk in range(self.unknowns):
          self.result[i+1,uk,...] = result[uk,...]*self.model.uk_sc[uk]
