...
#         # gradnorm = (np.quantile(tmp, 1, axis=-1))#-np.quantile(tmp, 0.1, axis=-1))
#         gradnorm = np.mean(np.abs(grad), axis=-1)
...
#         gradnorm[gradnorm>1e3] = 1e3
#         scale = 1e3/gradnorm/self.unknowns
#         scale[~np.isfinite(scale)] = 1e3/self.unknowns
#         # print("Scale: ", scale)
