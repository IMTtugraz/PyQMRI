...
__kernel void update_primal_LM(__global float2 *u_new, __global float2 *u, __global float2 *Kyk,__global float2 *u_k,
  ...
  for (int uk=0; uk<NUk; uk++)
  {
     Asqr = A[uk*Nx*Ny*NSl+i].x*A[uk*Nx*Ny*NSl+i].x + A[uk*Nx*Ny*NSl+i].y*A[uk*Nx*Ny*NSl+i].y;
...
__kernel void update_Kyk1(__global float2 *out, __global float2 *in,
                       ...
                       const int NScan, __global float* ratio, const int Nuk, const float dz)
{
  size_t X = get_global_size(2);
  size_t Y = get_global_size(1);
  size_t NSl = get_global_size(0);
...
__kernel void operator_fwd_cg(__global float2 *out, __global float2 *in,
                       ...
                       const int NScan)
{
  ...
  float2 tmp_mul = 0.0f;
    ...
    for (int scan=0; scan<NScan; scan++)
    {
      for (int coil=0; coil < NCo; coil++)
      {
        out[scan*NCo*NSl*X*Y+coil*NSl*X*Y+k*X*Y + y*X + x] = (float2)(in[scan*NSl*X*Y+k*X*Y+ y*X + x].x*coils[coil*NSl*X*Y + k*X*Y + y*X + x].x-
...
__kernel void operator_ad_cg(__global float2 *out, __global float2 *in,
                       __global float2 *coils, const int NCo,
                       const int NScan)
{
  size_t X = get_global_size(2);
