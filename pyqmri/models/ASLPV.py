...
class Model(BaseModel):
    def __init__(self, par):
        ...
        self.hard_wm_mask = np.zeros_like(self.wm_mask)
        self.hard_wm_mask[self.wm_mask>0] = 1
        self.hard_gm_mask = np.zeros_like(self.gm_mask)
        self.hard_gm_mask[self.gm_mask>0] = 1
    ...
    def _execute_forward_GM(self, x):
        f = x[0, ...] * self.uk_scale[2]
        del_t = x[1, ...] * self.uk_scale[3]

        S = np.zeros((self.NScan, self.NSlice, self.dimY, self.dimX),
                     dtype=self._DTYPE)

        T1prinv = _T1pr(self.T1, x[0], self.uk_scale[2], self.lambd)
        expAtt = _expAttT1b(x[1], self.uk_scale[3], self.T1b)
        for j in range(self.t.shape[0]):
            ind_low = self.t[j] >= del_t
            ind_high = self.t[j] < (del_t+self.tau[j])
            ind = ind_low & ind_high
    ...
    def _execute_forward_3D(self, x):
        S1 = self._execute_forward_WM(x[:2])
        S2 = self._execute_forward_GM(x[2:])

        return np.array(S1*self.wm_mask + S2*self.gm_mask, dtype=self._DTYPE)
    
    def _execute_gradient_WM(self, x):
        f_sc = self.uk_scale[0]
        del_t_sc = self.uk_scale[1]
        del_t = x[1]*del_t_sc
        grad = np.zeros((2, self.NScan,
    ...
    def _execute_gradient_GM(self, x):
        f_sc = self.uk_scale[2]
        del_t_sc = self.uk_scale[3]
        del_t = x[1]*del_t_sc
        grad = np.zeros((2, self.NScan,
                         self.NSlice, self.dimY, self.dimX), dtype=self._DTYPE)
        t = self.t
    ...
    def _execute_gradient_3D(self, x):
        g1 = self._execute_gradient_WM(x[:2])*self.wm_mask
        g2 = self._execute_gradient_GM(x[2:])*self.gm_mask

        return np.concatenate((g1,g2), axis=0).astype(self._DTYPE)
...
#        del_t[f <= 15] = 0
        ...
        f_min_gm = fgm.min()
        f_max_gm = fgm.max()
        del_t_min_gm = del_t_gm.min()
        del_t_max_gm = del_t_gm.max()
        # ind1 = 46
        ...
        if dim_2D:
            ...
            if not self._figure:
                ...
                self.gs = gridspec.GridSpec(
                    4, 12,
                    width_ratios=[
                        x / (20 * z), x / z, 1, x / z, 1, x / (20 * z), x / (20 * z), x / z, 1, x / z, 1, x / (20 * z)],
                ...
                self.f_plot = self.ax[1].imshow(
                    (f[int(self.NSlice / 2), ...]))
                self.ax[13].volume = np.swapaxes(f, 0, 1)
                self.ax[13].index = int(f.shape[1] / 2)
                self.f_plot_cor = self.ax[13].imshow(
                    (f[:, int(f.shape[1] / 2), ...]))
                ...
                self.ax[15].volume = np.swapaxes(del_t, 0, 1)
                self.ax[15].index = int(del_t.shape[1] / 2)
                self.del_t_plot_cor = self.ax[15].imshow(
                ...
                self.ax[7].volume = fgm
                self.ax[7].index = int(self.NSlice / 2)
                self.f_plot_gm = self.ax[7].imshow(
                    (fgm[int(self.NSlice / 2), ...]))
                self.ax[19].volume = np.swapaxes(fgm, 0, 1)
                self.ax[19].index = int(f.shape[1] / 2)
                self.f_plot_cor_gm = self.ax[19].imshow(
                    (fgm[:, int(f.shape[1] / 2), ...]))
                self.ax[8].volume = fgm.T
                self.ax[8].index = int(f.shape[-1] / 2)
                self.f_plot_sag_gm = self.ax[8].imshow(
                    np.flip((fgm[:, :, int(f.shape[-1] / 2)]).T, 1))
                self.ax[7].set_title('CBF GM', color='white')
                self.ax[7].set_anchor('SE')
                self.ax[8].set_anchor('SW')
                self.ax[19].set_anchor('NE')
                cax = plt.subplot(self.gs[:2, 6])
                cbar = self._figure.colorbar(self.f_plot, cax=cax)
                cbar.ax.tick_params(labelsize=12, colors='white')
                cax.yaxis.set_ticks_position('left')
                for spine in cbar.ax.spines:
                ...
                self.ax[9].volume = del_t_gm
                self.ax[9].index = int(self.NSlice / 2)
                self.del_t_plot_gm = self.ax[9].imshow(
                    (del_t_gm[int(self.NSlice / 2)+off, ...]))
                self.ax[21].volume = np.swapaxes(del_t_gm, 0, 1)
                self.ax[21].index = int(del_t.shape[1] / 2)
                self.del_t_plot_cor_gm = self.ax[21].imshow(
                    (del_t_gm[:, int(del_t.shape[1] / 2), ...]))
                self.ax[10].volume = del_t_gm.T
                self.ax[10].index = int(del_t.shape[-1] / 2)
                self.del_t_plot_sag_gm = self.ax[10].imshow(
                    np.flip((del_t_gm[:, :, int(del_t.shape[-1] / 2)]).T, 1))
                self.ax[9].set_title('ATT GM', color='white')
                self.ax[9].set_anchor('SE')
                self.ax[10].set_anchor('SW')
                self.ax[21].set_anchor('NE')
                cax = plt.subplot(self.gs[:2, 11])
                cbar = self._figure.colorbar(self.del_t_plot, cax=cax)
                cbar.ax.tick_params(labelsize=12, colors='white')
                for spine in cbar.ax.spines:
                    cbar.ax.spines[spine].set_color('white')
                ...
                self.ax[13].images[0].set_array(self.ax[13].volume[self.ax[13].index])
                ...
                self.ax[19].images[0].set_array(self.ax[19].volume[self.ax[19].index])
                
                self.f_plot_gm.set_clim([f_min_gm, f_max_gm])
                self.f_plot_cor_gm.set_clim([f_min_gm, f_max_gm])
                self.f_plot_sag_gm.set_clim([f_min_gm, f_max_gm])
                
                self.ax[9].images[0].set_array(self.ax[9].volume[self.ax[9].index])
                self.ax[10].images[0].set_array(self.ax[10].volume[self.ax[10].index])
                self.ax[21].images[0].set_array(self.ax[21].volume[self.ax[21].index])
                
                self.del_t_plot_gm.set_clim([del_t_min_gm, del_t_max_gm])
                self.del_t_plot_sag_gm.set_clim([del_t_min_gm, del_t_max_gm])
                self.del_t_plot_cor_gm.set_clim([del_t_min_gm, del_t_max_gm])
    ...
    def onclick(self, event):
        if event.inaxes in [self.ax[1], self.ax[3], self.ax[7], self.ax[9]]:
    ...
    def onscroll(self, event):
        if event.inaxes in [self.ax[1], self.ax[3], self.ax[7], self.ax[9]]:
            ...
            ax = [self.ax[1], self.ax[3], self.ax[7], self.ax[9]]
        
        elif event.inaxes in [self.ax[2], self.ax[4], self.ax[8], self.ax[10]]:
            fig = event.canvas.figure
            ax = [self.ax[2], self.ax[4], self.ax[8], self.ax[10]]
                        
        elif event.inaxes in [self.ax[13], self.ax[15], self.ax[19], self.ax[21]]:
            fig = event.canvas.figure
            ax = [self.ax[13], self.ax[15], self.ax[19], self.ax[21]]
        else:
            return
    ...
    def computeInitialGuess(self, *args):
        ...
        test_f = self.dscale * np.ones(
            (self.NSlice, self.dimY, self.dimX), dtype=self._DTYPE)
        test_del_t = 1/60 * np.ones(
            (self.NSlice, self.dimY, self.dimX), dtype=self._DTYPE)
        x = np.array([test_f*30*self.hard_wm_mask,
                      test_del_t*self.hard_wm_mask, test_f*30*self.hard_gm_mask, test_del_t*self.hard_gm_mask], dtype=self._DTYPE)
        self.guess = x
