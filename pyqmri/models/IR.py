...
class Model(BaseModel):
    ...
    def __init__(self, par):
        ...
        for i in range(self.NScan):
            self.TE[i, ...] = par["TI"][i] * np.ones((1, 1, 1))
    ...
    def _execute_gradient_3D(self, x):
        ...
        grad_M0 = self.uk_scale[0] * (1 - 2*np.exp(-self.TE * (
                T21 * self.uk_scale[1])))

        grad_T21 = 2*self.uk_scale[0] * M0 * \
