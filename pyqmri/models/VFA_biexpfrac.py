...
#    print('Grad Scaling init', np.linalg.norm(np.abs(DG_x[0,...]))/np.linalg.norm(np.abs(DG_x[1,...])))
    ...
    def execute_gradient_2D(self, x, islice):
        ...
        T21 = x[2, ...]
        T22 = x[3, ...]
        grad_M0 = self.uk_scale[0] * (M01 * self.uk_scale[1] * np.exp(-self.TE * (T21 * self.uk_scale[2])) + (
            1 - M01 * self.uk_scale[1]) * np.exp(-self.TE * (T22 * self.uk_scale[3])))
        grad_M01 = self.uk_scale[0] * M0 * (self.uk_scale[1] * np.exp(-self.TE * (
            T21 * self.uk_scale[2])) - self.uk_scale[1] * np.exp(-self.TE * (T22 * self.uk_scale[3])))
        grad_T21 = -self.uk_scale[0] * M0 * M01 * self.uk_scale[1] * self.TE * \
            self.uk_scale[2] * np.exp(-self.TE * (T21 * self.uk_scale[2]))
        grad_T22 = -self.uk_scale[0] * M0 * (1 - M01 * self.uk_scale[1]) * \
            self.TE * self.uk_scale[3] * np.exp(-self.TE * (T22 * self.uk_scale[3]))
    ...
    def plot_unknowns(self, x, dim_2D=False):
        ...
        if dim_2D:
            ...
            if not self.figure:
                ...
                self.T21_plot = self.ax[3].imshow(
                    (T21[int(self.NSlice / 2), ...]))
                self.T21_plot_cor = self.ax[16].imshow(
                    (T21[:, int(T21.shape[1] / 2), ...]))
                self.T21_plot_sag = self.ax[4].imshow(
                ...
                self.T22_plot = self.ax[10].imshow(
                    (T22[int(self.NSlice / 2), ...]))
                self.T22_plot_cor = self.ax[23].imshow(
                    (T22[:, int(T22.shape[1] / 2), ...]))
                self.T22_plot_sag = self.ax[11].imshow(
