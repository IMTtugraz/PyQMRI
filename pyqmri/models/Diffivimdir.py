...
class Model(BaseModel):
    ...
    def _execute_forward_3D(self, x):
        ADC1 = x[2, ...] * self.uk_scale[2] * self.dir[..., 0]**2 + \
            x[4, ...] * self.uk_scale[4] * self.dir[..., 1]**2 + \
            x[6, ...] * self.uk_scale[6] * self.dir[..., 2]**2 +\
            2 * x[3, ...] * self.uk_scale[3] * \
            self.dir[..., 0] * self.dir[..., 1] + \
            2 * x[5, ...] * self.uk_scale[5] * \
            self.dir[..., 0] * self.dir[..., 2] +\
            2 * x[7, ...] * self.uk_scale[7] * \
            self.dir[..., 1] * self.dir[..., 2]
        ADC2 = x[8, ...] * self.uk_scale[8] * self.dir[..., 0]**2 + \
            x[10, ...] * self.uk_scale[10] * self.dir[..., 1]**2 + \
            x[12, ...] * self.uk_scale[12] * self.dir[..., 2]**2 + \
            2 * x[9, ...] * self.uk_scale[9] * \
            self.dir[..., 0] * self.dir[..., 1] + \
            2 * x[11, ...] * self.uk_scale[11] * \
            self.dir[..., 0] * self.dir[..., 2] + \
            2 * x[13, ...] * self.uk_scale[13] * \
            self.dir[..., 1] * self.dir[..., 2]

        S = x[0, ...] * self.uk_scale[0] * (
            x[1, ...] * self.uk_scale[1] * np.exp(-self.b * (ADC1+ADC2)) +
            (1-x[1, ...] * self.uk_scale[1])*np.exp(- ADC1 * self.b))
    ...
    def _execute_gradient_3D(self, x):
        ...
        grad_f = x[0, ...] * self.uk_scale[0] * (
            -self.uk_scale[1] * np.exp(-ADC1 * self.b) +
            self.uk_scale[1] * np.exp(-self.b * (ADC1 + ADC2))) * self.phase

        exp1 = np.exp(-self.b*(ADC1+ADC2))
        exp2 = np.exp(-self.b*ADC1)

        del ADC1, ADC2

        grad_ADC1_x = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[2] * self.dir[..., 0]**2 * self.b *
             (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (- self.uk_scale[2] * self.dir[..., 0]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC1_xy = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[3] * self.dir[..., 0] * self.dir[..., 1] *
             self.b * (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (-2 * self.uk_scale[3] * self.dir[..., 0] * self.dir[..., 1] *
             self.b * x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC1_y = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[4] * self.dir[..., 1]**2 * self.b *
             (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (- self.uk_scale[4] * self.dir[..., 1]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC1_xz = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[5] * self.dir[..., 0] * self.dir[..., 2] *
             self.b * (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (-2 * self.uk_scale[5] * self.dir[..., 0] * self.dir[..., 2] *
             self.b*x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC1_z = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[6] * self.dir[..., 2]**2 * self.b *
             (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (- self.uk_scale[6] * self.dir[..., 2]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC1_yz = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[7] * self.dir[..., 1] * self.dir[..., 2] *
             self.b * (1-x[1, ...] * self.uk_scale[1])*exp2) +
            (-2 * self.uk_scale[7] * self.dir[..., 1] * self.dir[..., 2] *
             self.b*x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_x = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[8] * self.dir[..., 0]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_xy = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[9] * self.dir[..., 0] * self.dir[..., 1] *
             self.b * x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_y = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[10] * self.dir[..., 1]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_xz = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[11] * self.dir[..., 0] * self.dir[..., 2] *
             self.b*x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_z = x[0, ...] * self.uk_scale[0] * (
            (- self.uk_scale[12] * self.dir[..., 2]**2 * self.b *
             x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        grad_ADC2_yz = x[0, ...] * self.uk_scale[0] * (
            (-2 * self.uk_scale[13] * self.dir[..., 1] * self.dir[..., 2] *
             self.b*x[1, ...] * self.uk_scale[1]*exp1)) * self.phase

        del exp1, exp2

        grad = np.array([grad_M0, grad_f, grad_ADC1_x, grad_ADC1_xy,
                         grad_ADC1_y, grad_ADC1_xz, grad_ADC1_z,
                         grad_ADC1_yz, grad_ADC2_x, grad_ADC2_xy,
                         grad_ADC2_y, grad_ADC2_xz, grad_ADC2_z,
                         grad_ADC2_yz], dtype=DTYPE)
        grad[~np.isfinite(grad)] = 0
        return grad

    def plot_unknowns(self, x, dim_2D=False):
        M0 = np.abs(x[0, ...]) * self.uk_scale[0]
        ADC_x = (np.real(x[2, ...]) * self.uk_scale[2])
        ADC_xy = (np.real(x[3, ...]) * self.uk_scale[3])
        M0_min = M0.min()
        M0_max = M0.max()
        ADC_x_min = ADC_x.min()
        ADC_x_max = ADC_x.max()
        ...
        ADC_y = (np.real(x[4, ...]) * self.uk_scale[4])
        ADC_xz = (np.real(x[5, ...]) * self.uk_scale[5])
        ADC_y_min = ADC_y.min()
        ADC_y_max = ADC_y.max()
        ADC_xz_min = ADC_xz.min()
        ADC_xz_max = ADC_xz.max()

        ADC_z = (np.real(x[6, ...]) * self.uk_scale[6])
        ADC_yz = (np.real(x[7, ...]) * self.uk_scale[7])
        ADC_z_min = ADC_z.min()
        ADC_z_max = ADC_z.max()
