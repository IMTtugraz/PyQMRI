...
class Model(BaseModel):
    def __init__(self, par):
        ...
        par["unknowns_TGV"] = self.numC + len(self.b) + len(self.b)
        par["unknowns_H1"] = 0
    ...
    def _execute_forward_3D(self, x):
        ...
        for j in range(self.numT1Scale):
            ...
            S[offset*(j):offset*(j+1)] = (
                - self.b0*x[np.mod(j,self.numC)] * self.uk_scale[np.mod(j,self.numC)] * np.exp(-t / (x[-self.numT1Scale+j] * self.uk_scale[-self.numT1Scale+j]))
                + (1 - np.exp(-t / (x[-self.numT1Scale+j] * self.uk_scale[-self.numT1Scale+j])))
                * x[self.numC+j]*self.uk_scale[self.numC+j]*self.b[j]
                )
    ...
    def _gradC(self, x):
        ...
        for j in range(self.numT1Scale):
            ...
            grad[np.mod(j,self.numC), offset*(j):offset*(j+1)] = (
                self.uk_scale[np.mod(j,self.numC)]*self.b0
    ...
    def _gradAlpha(self, x):
        ...
        for j in range(self.numT1Scale):
            ...
            grad[j, offset*(j):offset*(j+1)] = (
                self.uk_scale[self.numC+j] *self.b[j]
                * (1-
                   np.exp(-t / (x[-self.numT1Scale+j] * self.uk_scale[-self.numT1Scale+j]))
                   )
                )
    ...
    def _gradT1(self, x):
        ...
        for j in range(self.numT1Scale):
            ...
            grad[j, (j)*offset:(j+1)*offset] = (
                x[np.mod(j,self.numC)]*self.uk_scale[np.mod(j,self.numC)]*self.b0*(
                    -t
                    * np.exp(-t/(x[-self.numT1Scale+j]*self.uk_scale[-self.numT1Scale+j])))
                -(t*x[self.numC+j]*self.uk_scale[self.numC+j]*self.b[j]
                    * np.exp(-t/(x[-self.numT1Scale+j]*self.uk_scale[-self.numT1Scale+j]))
                    )
    ...
    def plot_unknowns(self, x, dim_2D=False):
        ...
        tmp_x[:self.numT1Scale+self.numC] = np.abs(tmp_x[:self.numT1Scale+self.numC])/self.dscale
        tmp_x = np.abs(tmp_x)
    ...
    def computeInitialGuess(self, *args):
        ...
        for j in range(self.numT1Scale):
            test_Xi.append(
                ...
                np.ones(
                    (self.NSlice, self.dimY, self.dimX), dtype=self._DTYPE))
            self.constraints[j+self.numC].update(1/args[1])
