...
class Model(BaseModel):
    ...
    def computeInitialGuess(self, **kwargs):
        ...
        if np.allclose(kwargs['initial_guess'],-1):
            #default setting
            test_M0 = 1 * np.ones(kwargs['images'].shape[-3:], dtype=self._DTYPE)
            test_T11 = 1/100 * np.ones(kwargs['images'].shape[-3:], dtype=self._DTYPE)
        else:
            #custom initial guess
            assert len(kwargs['initial_guess']) == self.unknowns-1
            test_M0 = kwargs['initial_guess'][0]*np.ones(kwargs['images'].shape[-3:], 
                                                         dtype=self._DTYPE)
            test_T11 = kwargs['initial_guess'][1]*np.ones(kwargs['images'].shape[-3:], 
                                                          dtype=self._DTYPE)
