...
def _S(M0, M0sc, ADC, meanADC, kurt, b):
    return ne.evaluate("M0*M0sc*\
                       exp(-b*ADC+1/6*b**2*meanADC**2*kurt -\
                           1/27*b**3*meanADC**4/ADC*kurt**2)")


def _diffexp(ADC, meanADC, kurt, b):
    return ne.evaluate("exp(-b*ADC+1/6*b**2*meanADC**2*kurt \
                           - 1/27*b**3*meanADC**4/ADC*kurt**2)")
...
def _gradKurtDia(Kurtscale, bdir, comKurtfac1, comKurtfac2):
    return ne.evaluate("Kurtscale*(-2/27*bdir**4*comKurtfac1 + \
                                   1/6*bdir**4)*comKurtfac2")


def _gradKurtOff2(Kurtscale, bdir1, bdir2, comKurtfac1, comKurtfac2):
    return ne.evaluate("Kurtscale*(-8/27*bdir1**3*bdir2*comKurtfac1 + \
                                   2/3*bdir1**3*bdir2)*comKurtfac2")


def _gradKurtOffSym(Kurtscale, bdir1, bdir2, comKurtfac1, comKurtfac2):
    return ne.evaluate("Kurtscale*(-4/9*bdir1**2*bdir2**2*comKurtfac1 + \
                                   bdir1**2*bdir2**2)*comKurtfac2")


def _gradKurtOff3(Kurtscale, bdir1, bdir2, bdir3, comKurtfac1, comKurtfac2):
    return ne.evaluate("Kurtscale*(-8/9*bdir1**2*bdir2*bdir3*comKurtfac1 + \
                                   2*bdir1**2*bdir2*bdir3)*comKurtfac2")
...
class Model(BaseModel):
    ...
    def __init__(self, par, images):
        ...
        for j in range(6):
            self.constraints.append(constraints(
                (-1 / self.uk_scale[j + 10]),
                (1 / self.uk_scale[j + 10]), True))
        ...
        for j in range(3):
            self.constraints.append(constraints(
                (-1 / self.uk_scale[j + 19]),
                (1 / self.uk_scale[j + 19]), True))
    ...
    def _execute_gradient_3D(self, x):
        ...
        tmp1 = self.b**3*(meanADC)**3*(kurt)**2/ADC
        tmp2 = self.b**3*(meanADC)**4*(kurt)**2/ADC**2
        tmp3 = self.b**2*(meanADC)*(kurt)

        tmpM0 = x[0]*grad_M0

        grad_ADC_x = tmpM0*(-8/81*x[1]*self.uk_scale[1]**2*tmp1 + 2/9*x[1]*self.uk_scale[1]**2*tmp3 - 1/27*(-2*x[1]*self.uk_scale[1]**2*self.dirsqr[..., 0] - 2*self.uk_scale[1]*x[2]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] - 2*self.uk_scale[1]*x[4]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2])*tmp2 - self.b*(2*x[1]*self.uk_scale[1]**2*self.dirsqr[..., 0] + 2*self.uk_scale[1]*x[2]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] + 2*self.uk_scale[1]*x[4]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2]))

        grad_ADC_xy = tmpM0*(-8/81*x[2]*self.uk_scale[2]**2*tmp1 + 2/9*x[2]*self.uk_scale[2]**2*tmp3 - 1/27*(-2*x[1]*self.uk_scale[1]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] - 2*x[2]*self.uk_scale[2]**2*self.dirsqr[..., 1] - 2*self.uk_scale[2]*x[4]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2])*tmp2 - self.b*(2*x[1]*self.uk_scale[1]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] + 2*x[2]*self.uk_scale[2]**2*self.dirsqr[..., 1] + 2*self.uk_scale[2]*x[4]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2]))

        grad_ADC_y = tmpM0*(-8/81*x[3]*self.uk_scale[3]**2*tmp1 + 2/9*x[3]*self.uk_scale[3]**2*tmp3 - 1/27*(-2*x[3]*self.uk_scale[3]**2*self.dirsqr[..., 1] - 2*self.uk_scale[3]*x[6]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2])*tmp2 - self.b*(2*x[3]*self.uk_scale[3]**2*self.dirsqr[..., 1] + 2*self.uk_scale[3]*x[6]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2]))

        grad_ADC_xz = tmpM0*(-8/81*x[4]*self.uk_scale[4]**2*tmp1 + 2/9*x[4]*self.uk_scale[4]**2*tmp3 - 1/27*(-2*x[1]*self.uk_scale[1]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2] - 2*x[2]*self.uk_scale[2]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2] - 2*x[4]*self.uk_scale[4]**2*self.dirsqr[..., 2])*tmp2 - self.b*(2*x[1]*self.uk_scale[1]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2] + 2*x[2]*self.uk_scale[2]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2] + 2*x[4]*self.uk_scale[4]**2*self.dirsqr[..., 2]))

        grad_ADC_z = tmpM0*(2/27*x[5]*self.uk_scale[5]**2*self.dirsqr[..., 2]*tmp2 - 8/81*x[5]*self.uk_scale[5]**2*tmp1 + 2/9*x[5]*self.uk_scale[5]**2*tmp3 - 2*x[5]*self.uk_scale[5]**2*self.b*self.dirsqr[..., 2])

        grad_ADC_yz = tmpM0*(-8/81*x[6]*self.uk_scale[6]**2*tmp1 + 2/9*x[6]*self.uk_scale[6]**2*tmp3 - 1/27*(-2*x[3]*self.uk_scale[3]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2] - 2*x[6]*self.uk_scale[6]**2*self.dirsqr[..., 2])*tmp2 - self.b*(2*x[3]*self.uk_scale[3]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2] + 2*x[6]*self.uk_scale[6]**2*self.dirsqr[..., 2]))

        tmp1 = self.b*(meanADC)**2*(kurt)/(ADC)
        tmp2 = tmpM0*self.b**2*(meanADC)**2
