...
from pyqmri.models.template import BaseModel, constraints, DTYPE, DTYPE_real
...
#    self.constraints.append(constraints(0,1e6/self.uk_scale[0],False)  )
        ...
        self.constraints.append(
            constraints(
                (self.TR / 60 * par["dimY"]) / self.uk_scale[2],
                100 / self.uk_scale[2],
                True))
        self.constraints.append(
            constraints(
                1e-4 / self.uk_scale[3],
                3 / self.uk_scale[3],
                True))

        self.constraints.append(
            constraints(
                (self.TR / 60 * par["dimY"]) / self.uk_scale[4],
                3 / self.uk_scale[4],
                True))
        self.constraints.append(
            constraints(
...
#    self.constraints.append(constraints(1e-4/self.uk_scale[9],3/self.uk_scale[9],True))
    ...
    def _execute_forward_3D(self, x):
        ...
        S = x[0] * self.uk_scale[0] * \
            (-E1 + 1) * self.sin_phi / (-E1 * self.cos_phi + 1)
    ...
    def _execute_gradient_3D(self, x):
        ...
        grad_M0 = self.uk_scale[0] * (-E1 + 1) * \
            self.sin_phi / (-E1 * self.cos_phi + 1)

        grad_T1 = self.r * M0 * M0_sc * (self.TR * E1 * self.sin_phi / (1 - E1 * self.cos_phi) - self.TR * (
            1 - E1) * E1 * self.sin_phi * self.cos_phi / (1 - E1 * self.cos_phi)**2)
        grad_T1 = grad_T1 * dC
...
#    print('Grad Scaling tau', np.linalg.norm(np.abs(grad[0]))/np.linalg.norm(np.abs(grad[5])))
    ...
    def plot_unknowns(self, x, dim_2D=False):
        ...
        Fp = np.concatenate(
            (np.abs(x[1, ...] * self.uk_scale[1]), self.FP), axis=-1)
        M0_min = M0.min()
        M0_max = M0.max()
        Fp_min = 0  # Fp.min()
        Fp_max = 1  # Fp.max()
        Te = np.concatenate(
            (np.abs(x[2, ...] * self.uk_scale[2]), self.Te), axis=-1)
        alpha = np.concatenate(
            (np.abs(x[3, ...] * self.uk_scale[3]), self.alpha), axis=-1)
        Te_min = 0  # Te.min()
        Te_max = 5  # Te.max()
        alpha_min = alpha.min()
        alpha_max = alpha.max()
        Tc = np.concatenate(
            (np.abs(x[4, ...] * self.uk_scale[4]), self.Tc), axis=-1)
        tau = np.concatenate(
            (np.abs(x[5, ...] * self.uk_scale[5]), self.tau), axis=-1)
        Tc_min = Tc.min()
        Tc_max = Tc.max()
        tau_min = tau.min()
        tau_max = tau.max()
...
#           self.figure.tight_layout()
            ...
            if not self.figure:
                ...
                self.time_course_ref = self.ax[57].plot(
                    self.t, np.abs(self.images[:, 0, 82, 52]), 'g')[0]
                self.time_course = self.ax[57].plot(
                    self.t, np.abs(images[:, 0, 82, 52]), 'r')[0]
                self.ax[57].set_ylim(np.abs(self.images[:,
                                                        0,
                                                        82,
                                                        52]).min() - np.abs(self.images[:,
                                                                                        0,
                                                                                        82,
                                                                                        52]).min() * 0.01,
                                     np.abs(self.images[:,
...
#        dC_fit[6,j,ind]=(t53-A_G[ind]*t33)*self.uk_scale[7]
                dC_fit[-1, j, ind] = (t65 + t73 - A_B[ind]
                                      * mu_B[ind] * t5 * t29) * self.uk_scale[5]
...
#        t53 = A_G[ind]*t52
                ...
                dC_fit[:, j, ind] = np.array((
                    ...
                    self.uk_scale[2] * (-t13 * (A_G[ind] * (t13 * t16 * t23 * t25 * t54 - t13 * t16 * t27 * t49 * t54) - A_B[ind] * t23 * (t13 * t16 * t39 * t54 * 2.0 - t13 * t14 * t16 * t25 * t54) - A_G[ind] * t24 * (
                    ...
                    self.uk_scale[3] * (t12 * (t63 + A_G[ind] * (t46 - t62) - A_B[ind] * t23 * t34 - A_G[ind] * t24 * t47) - t13 * (A_G[ind] * (t23 * t25 * t58 - t27 * t49 * t58) - A_G[ind] * t24 * (
...
# test_M0 = par['file']['PD_est'][()].astype(DTYPE)[None,...]*dscale#2Coil
# 1.0290821 #### 4 Coil 1.0189297

        FP = 0.1 * np.ones((self.NSlice, self.dimY, self.dimX),
                           dtype=DTYPE) * self.mask  # parameters[4][...]# 0.5
        # parameters[6][...]#5*np.ones((NSlice,dimY,dimX),dtype=DTYPE)#
        Te = 2.5 * np.ones((self.NSlice, self.dimY, self.dimX),
                           dtype=DTYPE) * self.mask

#    Te[Te<1/60] = 1/60
        alpha = 0.2 * np.ones((self.NSlice, self.dimY, self.dimX),
                              dtype=DTYPE) * self.mask  # parameters[7][...]#
...
# 0.1*np.ones((self.NSlice,self.dimY,self.dimX),dtype=DTYPE)*self.mask#parameters[4][...]#
        Tc = 1 / 6 * np.ones((self.NSlice, self.dimY, self.dimX),
                             dtype=DTYPE) * self.mask  # parameters[4][...]#
        tau = 0.5 * np.ones((self.NSlice, self.dimY, self.dimX),
                            dtype=DTYPE) * self.mask  # parameters[4][...]#
