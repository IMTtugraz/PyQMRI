...
def _S(M0, M0sc, ADC, meanADC, kurt, b):
    return ne.evaluate("M0*M0sc*(1+kurt*meanADC**2*b/(3*ADC))**\
                       (-3/(meanADC**2/ADC**2*kurt))")


def _diffexp(ADC, meanADC, kurt, b):
    return ne.evaluate("(1+kurt*meanADC**2*b/(3*ADC))**\
                       (-3/(meanADC**2/ADC**2*kurt))")


def _gradKurtDia(Kurtscale, bdir, comKurtfac):
    return ne.evaluate("Kurtscale*bdir**4*comKurtfac")


def _gradKurtOff2(Kurtscale, bdir1, bdir2, comKurtfac):
    return ne.evaluate("4*Kurtscale*bdir1**3*bdir2*comKurtfac")


def _gradKurtOffSym(Kurtscale, bdir1, bdir2, comKurtfac):
    return ne.evaluate("6*Kurtscale*bdir1**2*bdir2**2*comKurtfac")


def _gradKurtOff3(Kurtscale, bdir1, bdir2, bdir3, comKurtfac):
    return ne.evaluate("12*Kurtscale*bdir1**2*bdir2*bdir3*comKurtfac")
...
class Model(BaseModel):
    ...
    def _execute_gradient_3D(self, x):
        ...
        tmpdenum = (3*(x[1]*self.uk_scale[1])**2*self.dirsqr[..., 0] +
                    6*x[1]*self.uk_scale[1] *
                    x[2]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] +
                    6*x[1]*self.uk_scale[1] *
                    x[4]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2] +
                    3*self.dirsqr[..., 1]*((x[2]*self.uk_scale[2])**2 +
                                           (x[3]*self.uk_scale[3])**2) +
                    3*self.dir[..., 1]*self.dir[..., 2]*(
                        2*x[2]*self.uk_scale[2]*x[4]*self.uk_scale[4] +
                        2*x[3]*self.uk_scale[3]*x[6]*self.uk_scale[6]) +
                    3*self.dirsqr[..., 2]*(
                        (x[4]*self.uk_scale[4])**2 +
                        (x[6]*self.uk_scale[6])**2 +
                        (x[5]*self.uk_scale[5])**2))

        tmpdenum2 = ((self.b*(meanADC)**2*(kurt)/tmpdenum + 1) *
                     (meanADC)**2*(kurt))

        logtmp = np.log(self.b*(meanADC)**2*(kurt)/tmpdenum + 1)

        com_diff_fac1 = 4*(ADC)**2/((meanADC)**3*(kurt))*logtmp
        com_diff_fac2 = 12*(ADC)/((meanADC)**2*(kurt))*logtmp
        com_diff_fac3 = (1+1/3)*3*self.b*(meanADC)*(kurt) /\
            tmpdenum*(ADC)**2/tmpdenum2
        com_diff_fac4 = (6*self.b*(meanADC)**2*(kurt)/tmpdenum**2) *\
            (ADC)**2/tmpdenum2


        grad_ADC_x = S0*((x[1]*self.uk_scale[1]**2*com_diff_fac1 -
                             (x[1]*self.uk_scale[1]**2*self.dirsqr[..., 0] +
                              self.uk_scale[1]*x[2]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] +
                              self.uk_scale[1]*x[4]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2])*com_diff_fac2) -
                            (x[1]*self.uk_scale[1]**2*com_diff_fac3 +
                             (-x[1]*self.uk_scale[1]**2*self.dirsqr[..., 0] -
                              self.uk_scale[1]*x[2]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] -
                              self.uk_scale[1]*x[4]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2])*com_diff_fac4))

        grad_ADC_xy = S0*((x[2]*self.uk_scale[2]**2*com_diff_fac1 -
                              (x[1]*self.uk_scale[1]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] +
                               x[2]*self.uk_scale[2]**2*self.dirsqr[..., 1] +
                               self.uk_scale[2]*x[4]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2])*com_diff_fac2) -
                             (x[2]*self.uk_scale[2]**2*com_diff_fac3 +
                              (-x[1]*self.uk_scale[1]*self.uk_scale[2]*self.dir[..., 0]*self.dir[..., 1] -
                               x[2]*self.uk_scale[2]**2*self.dirsqr[..., 1] -
                               self.uk_scale[2]*x[4]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2])*com_diff_fac4))

        grad_ADC_y = S0*((x[3]*self.uk_scale[3]**2*com_diff_fac1 -
                             (x[3]*self.uk_scale[3]**2*self.dirsqr[..., 1] +
                              self.uk_scale[3]*x[6]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2])*com_diff_fac2) -
                            (x[3]*self.uk_scale[3]**2*com_diff_fac3 +
                             (-x[3]*self.uk_scale[3]**2*self.dirsqr[..., 1] -
                              self.uk_scale[3]*x[6]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2])*com_diff_fac4))

        grad_ADC_xz = S0*((x[4]*self.uk_scale[4]**2*com_diff_fac1 -
                              (x[1]*self.uk_scale[1]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2] +
                               x[2]*self.uk_scale[2]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2] +
                               x[4]*self.uk_scale[4]**2*self.dirsqr[..., 2])*com_diff_fac2) -
                             (x[4]*self.uk_scale[4]**2*com_diff_fac3 +
                              (-x[1]*self.uk_scale[1]*self.uk_scale[4]*self.dir[..., 0]*self.dir[..., 2] -
                               x[2]*self.uk_scale[2]*self.uk_scale[4]*self.dir[..., 1]*self.dir[..., 2] -
                               x[4]*self.uk_scale[4]**2*self.dirsqr[..., 2])*com_diff_fac4))

        grad_ADC_z = S0*((-x[5]*self.uk_scale[5]**2*self.dirsqr[..., 2]*com_diff_fac2 +
                             x[5]*self.uk_scale[5]**2*com_diff_fac1) -
                            (-3*x[5]*self.uk_scale[5]**2*self.dirsqr[..., 2]*com_diff_fac4 +
                               x[5]*self.uk_scale[5]**2*com_diff_fac3))

        grad_ADC_yz = S0*((x[6]*self.uk_scale[6]**2*com_diff_fac1 -
                              (x[3]*self.uk_scale[3]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2] +
                               x[6]*self.uk_scale[6]**2*self.dirsqr[..., 2])*com_diff_fac2) -
                             (x[6]*self.uk_scale[6]**2*com_diff_fac3 +
                              (-x[3]*self.uk_scale[3]*self.uk_scale[6]*self.dir[..., 1]*self.dir[..., 2] -
                               x[6]*self.uk_scale[6]**2*self.dirsqr[..., 2])*com_diff_fac4))

        del com_diff_fac1, com_diff_fac2, com_diff_fac3, \
            com_diff_fac4, tmpdenum2

        com_kurt_fac = S0*3*self.b*(ADC)**2*(
            -1/((self.b*(meanADC)**2*(kurt)/tmpdenum + 1) * tmpdenum*(kurt)) +
            logtmp/((meanADC)**2*(kurt)**2))

        del tmpdenum, meanADC, ADC, kurt, logtmp, S0
        ...
        grad_kurt_xxxy = _gradKurtOff2(self.uk_scale[10],
                                       self.dir[..., 0], self.dir[..., 1],
                                       com_kurt_fac)
        grad_kurt_xxxz = _gradKurtOff2(self.uk_scale[11],
                                       self.dir[..., 0], self.dir[..., 2],
                                       com_kurt_fac)
        grad_kurt_xyyy = _gradKurtOff2(self.uk_scale[12],
                                       self.dir[..., 1], self.dir[..., 0],
                                       com_kurt_fac)
        grad_kurt_yyyz = _gradKurtOff2(self.uk_scale[13],
                                       self.dir[..., 1], self.dir[..., 2],
                                       com_kurt_fac)
        grad_kurt_xzzz = _gradKurtOff2(self.uk_scale[14],
                                       self.dir[..., 2], self.dir[..., 0],
                                       com_kurt_fac)
        grad_kurt_yzzz = _gradKurtOff2(self.uk_scale[15],
                                       self.dir[..., 2], self.dir[..., 1],
                                       com_kurt_fac)

        grad_kurt_xxyy = _gradKurtOffSym(self.uk_scale[16],
                                         self.dir[..., 0], self.dir[..., 1],
                                         com_kurt_fac)
        grad_kurt_xxzz = _gradKurtOffSym(self.uk_scale[17],
                                         self.dir[..., 0], self.dir[..., 2],
                                         com_kurt_fac)
        grad_kurt_yyzz = _gradKurtOffSym(self.uk_scale[18],
                                         self.dir[..., 1], self.dir[..., 2],
                                         com_kurt_fac)

        grad_kurt_xxyz = _gradKurtOff3(self.uk_scale[19],
                                       self.dir[..., 0],
                                       self.dir[..., 1],
                                       self.dir[..., 2],
                                       com_kurt_fac)
        grad_kurt_xyyz = _gradKurtOff3(self.uk_scale[20],
                                       self.dir[..., 1],
                                       self.dir[..., 0],
                                       self.dir[..., 2],
                                       com_kurt_fac)
        grad_kurt_xyzz = _gradKurtOff3(self.uk_scale[21],
                                       self.dir[..., 2],
                                       self.dir[..., 0],
                                       self.dir[..., 1],
                                       com_kurt_fac)
