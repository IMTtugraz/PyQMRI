...
class Model(BaseModel):
    ...
    def computeInitialGuess(self, *args):
        ...
        if np.allclose(args[2],-1):
            ...
            ADC_ivim = 100 * np.ones(args[0].shape[-3:], dtype=self._DTYPE)
        else:
            assert len(args[2]) == self.unknowns-1
            
            ADC = args[2][0] * np.ones(args[0].shape[-3:], dtype=self._DTYPE)
            f = args[2][-2] * np.ones(args[0].shape[-3:], dtype=self._DTYPE)
            ADC_ivim = args[2][-1] * np.ones(args[0].shape[-3:], dtype=self._DTYPE)
