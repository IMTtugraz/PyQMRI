...
data_save = data
result = np.zeros((8,2,5,256,256),dtype=DTYPE)
result_ref = np.zeros((10,2,5,256,256),dtype=DTYPE)
for k in range(10):
  data = data_save[:,:,32-5+k,:,:]
  data = data[:,:,None,:,:]
  dimX = 256#192
  dimY = 256#192
  ...
  par.fa_corr = np.flip(par.fa_corr,axis=0)[32-5+k,:,:]
  ...
  opt.execute_2D()
  result[:,:,k,:,:] = np.copy(np.squeeze(opt.result))
...
################################################################################
  ...
  opt_t.execute_2D()
  result_ref[:,:,k,:,:] = np.copy(np.squeeze(opt_t.result))
...
dset_result = f.create_dataset("full_result",result.shape,dtype=np.complex64,data=result)
dset_result_ref = f.create_dataset("ref_full_result",result_ref.shape,dtype=np.complex64,data=result_ref)
